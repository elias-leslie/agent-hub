    ...
    <div className="mt-3 group">
      ...
      <button
        ...
        className={cn(
          ...
          "hover:shadow-sm hover:shadow-amber-950/50",
        )}
      >
        ...
        <div className="flex-1 flex items-center gap-2">
          <div className="flex-1 h-2 bg-amber-900/50 rounded-full overflow-hidden relative">
            {/* Model limit indicator (subtle background line) */}
            {limitPercent < 100 && (
              <div
                className="absolute h-full w-0.5 bg-amber-500/20 z-10"
          ...
          {/* Token count badge */}
          <span className="text-xs font-mono font-semibold text-amber-300 tabular-nums whitespace-nowrap">
            {formatNumber(outputTokens)}/{formatNumber(maxTokensRequested)}
          </span>
        </div>

        {/* Truncated label */}
        <div className="flex items-center gap-1 px-2 py-0.5 rounded bg-amber-800/30">
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-3 mb-2">
            <div className="space-y-0.5">
              <div className="text-amber-500 font-medium uppercase tracking-wide text-[9px]">
                Output
              </div>
              <div className="font-mono font-bold text-amber-200 tabular-nums">
          ...
          {/* Tip */}
          <div className="mt-2 pt-2 border-t border-amber-800/30 flex items-start gap-1.5">
            <span className="text-amber-600 text-[10px]">
              TIP
            </span>
            <p className="text-amber-400/70 text-[10px] leading-relaxed">
              Increase{" "}
              <code className="px-1 py-0.5 rounded bg-amber-800/30 font-mono">
                max_tokens
              </code>{" "}
