...
How much does the ball cost? Think through this very carefully and show your reasoning."""
    ...
    try:
        ...
        for level in thinking_levels:
            ...
            thinking_chars = (
                len(result.thinking_content) if result.thinking_content else 0
            )
            print(f"\n{level.upper()} thinking:")
            print(f"  Time: {elapsed:.1f}s ({int(elapsed * 1000)}ms)")
            print(
                f"  Output: {result.output_tokens} tokens, {len(result.content)} chars"
            )
            print(
                f"  Thinking: {result.thinking_tokens or 0} tokens, {thinking_chars} chars"
            )
