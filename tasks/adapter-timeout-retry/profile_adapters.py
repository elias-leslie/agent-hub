...
async def measure_response_time(
    adapter, model: str, prompt: str, label: str
) -> dict[str, Any]:
...
async def profile_adapter(
    adapter_name: str, adapter, model: str, prompts: dict[str, str]
):
    ...
    for label, prompt in prompts.items():
        ...
        if result["success"]:
            print(
                f"  ✓ {result['elapsed_ms']}ms ({result['output_tokens']} tokens, {result['chars']} chars)"
            )
        else:
...
async def main():
    ...
    if settings.anthropic_api_key or True:  # OAuth via CLI
        try:
            ...
            claude_results = await profile_adapter(
                "Claude Sonnet 4.5", claude_adapter, "claude-sonnet-4-5", prompts
            )
    ...
    if settings.gemini_api_key:
        try:
            ...
            gemini_results = await profile_adapter(
                "Gemini 3 Flash", gemini_adapter, "gemini-3-flash-preview", prompts
            )
    ...
    if all_times:
        ...
        p99 = (
            sorted(all_times)[int(len(all_times) * 0.99)]
            if len(all_times) > 1
            else max(all_times)
        )
        ...
        if recommended_timeout < current_plan:
            ...
            print(
                f"  Consider more aggressive timeout: {recommended_timeout / 1000:.0f}s"
            )
        else:
