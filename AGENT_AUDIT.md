...
- `task-d5e78dde` moved persona runtime hook implementation behind `usePersonaRuntimeCore.ts`; public `usePersonaRuntime.ts` is now a 7-line compatibility facade. Verification: Biome, full frontend TSC, focused persona Vitest, changed gate, and browser `/persona` checks passed; `st done` merged it to `main`.
