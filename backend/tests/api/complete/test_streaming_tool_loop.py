...
@pytest.mark.asyncio
async def test_iter_stream_sse_with_tools_runtime_session_keeps_heartbeat_closeout_with_progress_tags(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from app.api.complete import streaming_tool_loop as mod

    heartbeat_closeout = (
        "[[P:found:no changes needed; inspected live cleanup truth]]\n"
        "[[P:decision:kept sha closure blocked pending COMPLETE_READY yes]]\n"
        "HEARTBEAT_ACTION\n"
        "[[S:partial:no changes made; sha task still not closure-ready]]"
    )
    ...
    ctx = StreamContext(
        session_id="sess-heartbeat-progress-closeout",
        model="codex/gpt-5.4",
        provider="codex",
        agent_used="persona",
        model_used="codex/gpt-5.4",
