...
class TestGetOutputLimit:
    """Tests for output token limit retrieval."""

    def test_claude_sonnet_limit(self):
        """Test Claude Sonnet output limit."""
        from app.services.token_counter import get_output_limit

        limit = get_output_limit("claude-sonnet-4-5-20250514")
        assert limit == 64000

    def test_claude_opus_limit(self):
        """Test Claude Opus output limit."""
        from app.services.token_counter import get_output_limit

        limit = get_output_limit("claude-opus-4-5-20251101")
        assert limit == 64000

    def test_claude_haiku_limit(self):
        """Test Claude Haiku output limit."""
        from app.services.token_counter import get_output_limit

        limit = get_output_limit("claude-haiku-4-5")
        assert limit == 64000

    def test_gemini_flash_limit(self):
        """Test Gemini Flash output limit."""
        from app.services.token_counter import get_output_limit

        limit = get_output_limit("gemini-3-flash-preview")
        assert limit == 65536

    def test_gemini_pro_limit(self):
        """Test Gemini Pro output limit."""
        from app.services.token_counter import get_output_limit

        limit = get_output_limit("gemini-3-pro-preview")
        assert limit == 65536

    def test_unknown_model_default(self):
        """Test unknown model falls back to claude-sonnet-4 via _get_model_base."""
        from app.services.token_counter import get_output_limit

        limit = get_output_limit("unknown-model-xyz")
        # Falls back to claude-sonnet-4 which has 64000 output limit
        assert limit == 64000


class TestGetRecommendedMaxTokens:
    """Tests for recommended max_tokens by use case."""

    def test_chat_use_case(self):
        """Test chat use case returns OUTPUT_LIMIT_CHAT."""
        from app.services.token_counter import get_recommended_max_tokens

        tokens = get_recommended_max_tokens("claude-sonnet-4-5", "chat")
        assert tokens == 4096  # OUTPUT_LIMIT_CHAT

    def test_code_use_case(self):
        """Test code use case returns OUTPUT_LIMIT_CODE."""
        from app.services.token_counter import get_recommended_max_tokens

        tokens = get_recommended_max_tokens("claude-sonnet-4-5", "code")
        assert tokens == 16384  # OUTPUT_LIMIT_CODE

    def test_analysis_use_case(self):
        """Test analysis use case returns OUTPUT_LIMIT_ANALYSIS."""
        from app.services.token_counter import get_recommended_max_tokens

        tokens = get_recommended_max_tokens("claude-sonnet-4-5", "analysis")
        assert tokens == 32768  # OUTPUT_LIMIT_ANALYSIS

    def test_agentic_use_case(self):
        """Test agentic use case returns OUTPUT_LIMIT_AGENTIC."""
        from app.services.token_counter import get_recommended_max_tokens

        tokens = get_recommended_max_tokens("claude-sonnet-4-5", "agentic")
        assert tokens == 64000  # OUTPUT_LIMIT_AGENTIC

    def test_general_use_case(self):
        """Test general/default use case returns DEFAULT_OUTPUT_LIMIT."""
        from app.services.token_counter import get_recommended_max_tokens

        tokens = get_recommended_max_tokens("claude-sonnet-4-5", "general")
        assert tokens == 8192  # DEFAULT_OUTPUT_LIMIT

    def test_no_model_returns_use_case_default(self):
        """Test None model returns use case default."""
        from app.services.token_counter import get_recommended_max_tokens

        tokens = get_recommended_max_tokens(None, "code")
        assert tokens == 16384  # OUTPUT_LIMIT_CODE

    def test_respects_model_max(self):
        """Test that returned value doesn't exceed model max."""
        from app.services.token_counter import get_recommended_max_tokens

        # If a model had a lower limit than the use case default,
        # the function should return the model's max
        # All current models support 64K+, so agentic (64K) should work
        tokens = get_recommended_max_tokens("claude-sonnet-4-5", "agentic")
        assert tokens <= 64000
