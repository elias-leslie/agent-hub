...
- Phase 4 cluster D **wave 3 (REMAINING)**: complete_orchestrator.py and its helpers (`orchestration_helpers.py`, `handlers.py`, `handler_helpers.py`, `event_helpers.py`, `helpers.py`, `async_dispatch.py`, `execution.py`, `complete_execution.py`, `work_context.py`, `result_finalizer.py`) are still in the active call path for `POST /complete`. The convergence-map flagged most of these as REMOVE-THEATRE on the assumption that `orchestrator.run_completion`/`run_completion_stream` would absorb their concerns — but currently `orchestrator.py` is intentionally thin and does NOT know about validation, quotas, cooldowns, agent resolution, mandate injection, async dispatch, caching, ctx info, build_session_and_messages, or execute_and_respond. The remaining work is to either (a) fold those concerns into `orchestrator.py` / `session_repo.py` / `app.routing` and delete the helpers, or (b) keep `complete_orchestrator` as the thin HTTP wiring + delete only its DEAD helpers (`async_dispatch` is still used; `execution.py` is still used; `complete_execution._run_with_agentic_fallback` is still used by agentic + fallback paths). Option (b) is the lighter-touch refactor. The cluster D list of "theatre helpers" was over-stated and needs a per-file reachability audit.
- Phase 4 cluster C (delete `backend/app/adapters/`): **NOT STARTED**. Blockers: `app/services/orchestration/subagent.py` + `subagent_executor.py` still instantiate `ClaudeAdapter()`/`GeminiAdapter()` and call `adapter.complete()`. Migrating these to `complete_internal` is required before adapter provider files can be deleted. Image adapters (`cloudflare_image`, `gemini_image`, `minimax_image`, `nvidia_image`, `image_base`) stay. `base.py` re-exports `Message`/`CompletionResult`/`ProviderError`/`RateLimitError`/`AuthenticationError`/`ProviderAdapter` to many non-adapter consumers (`app/api/health_logic.py`, `app/api/orchestration_models.py`, `app/services/provider_chain.py`, `app/api/complete/execution.py`, `app/routing/resolution.py`, etc.) — these would need to either move to a new home or keep `base.py` as a stripped re-export module. The `tests/adapters/` directory has many tests of the legacy boundary; most can be deleted as REMOVE-THEATRE alongside their target.
- Phase 4 cluster E (vulture sweep): **NOT STARTED**. Run after C/D-wave-3 land.
