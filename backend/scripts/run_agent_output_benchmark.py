...
def derive_suite_id(agent_slug: str, case_ids: list[str]) -> str:
    ...
    if normalized == defaults:
        return f"output-contract-{agent_slug}"
    if len(normalized) == 1:
        return normalized[0]
...
async def _run_one_attempt(
    *,
    ...
    message_content = case.prompt if not explicit_model_override else f"@{requested_model}\n{case.prompt}"
...
async def run_agent_output_benchmark(
    *,
            ...
            for requested_model in requested_models:
                for case_id in case_ids:
                    attempt = await _run_one_attempt(
                        ...
                        timeout_seconds=timeout_seconds,
                        use_memory=use_memory,
                        memory_group_id=memory_group_id or f"agent-output-benchmark:{agent_slug}:{seed}:{run_number}",
                    )
                    attempts.append(attempt)

    resolved_models = sorted({attempt.requested_model or attempt.effective_model or attempt.model_id for attempt in attempts})
    return AgentOutputBenchmarkRun(
        ...
        attempts=attempts,
        summaries=summarize_output_contract_attempts(attempts),
    )
        ...
        normalized_attempts.append(
            {
                **attempt,
                ...
                "should_close": None,
                "confidence": None,
                "summary": attempt.get("summary_excerpt"),
...
async def main() -> None:
    ...
    parser.add_argument("--project-id", default="agent-hub")
    ...
    if not args.no_persist:
        ...
        payload = build_persistence_payload(
            ...
            metadata={
                "benchmark_type": "output_contract",
                "benchmark_agent_slug": args.agent_slug,
            },
