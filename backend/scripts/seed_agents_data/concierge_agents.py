...
CONCIERGE_AGENTS = [
    ...
    },
    {
        ...
        "system_prompt": (
            ...
            "- Memory tier architecture compliance (see rules below)\n\n"
            "Memory tier architecture rules (flag violations as memory_issue):\n"
            "- Mandates are universal — all agents see all mandates at full text (L2). "
            "No audience_tags filtering on mandates or guardrails.\n"
            "- Role-specific instructions belong in reference tier with appropriate "
            "audience_tags (e.g. persona-relevant, finance-relevant), not as mandates.\n"
            "- All mandates must be pinned (prevents automated tier demotion by tier optimizer).\n"
            "- Target mandate count: 25-30 consolidated episodes. Flag if count exceeds 40.\n"
            "- Flag unpinned mandates as drift — they should be pinned for demotion protection.\n"
            "- Flag mandates that are clearly role-specific (only relevant to one agent type) "
            "as candidates for demotion to reference tier.\n\n"
            "Boundaries:\n"
