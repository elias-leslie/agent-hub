...
class _ServiceSearchMixin:
    ...
    async def text_search(
        ...
        return await search_ops.text_search(
            group_id, self.scope, self.scope_id, query, limit, category  # type: ignore[attr-defined]
        )
    ...
    async def get_context_for_query(
        ...
        return await search_ops.get_query_context(
            self._group_id, self.scope, query, max_facts, max_entities  # type: ignore[attr-defined]
        )
    ...
    async def get_patterns_and_gotchas(
        ...
        return await search_ops.get_patterns_gotchas(
            self._group_id, self.scope, query, num_results, min_score  # type: ignore[attr-defined]
        )
    ...
    async def get_session_history(self, num_sessions: int = 5) -> list[MemoryEpisode]:
        ...
        return await search_ops.get_history(
            self._group_id, self.scope, self.scope_id, num_sessions  # type: ignore[attr-defined]
        )
