...
def _make_attempt(run: AgentBenchmarkRun, attempt_payload: dict[str, Any]) -> AgentBenchmarkAttempt:
    return AgentBenchmarkAttempt(
        ...
        task_family=attempt_payload.get("task_family"),
        harness_arm=attempt_payload.get("harness_arm"),
        dimension_scores=dict(attempt_payload.get("dimension_scores") or {}),
        runtime_metrics=dict(attempt_payload.get("runtime_metrics") or {}),
        safety_failures=list(attempt_payload.get("safety_failures") or []),
        oracle_details=dict(attempt_payload.get("oracle_details") or {}),
        artifact_identity=dict(attempt_payload.get("artifact_identity") or {}),
        input_sha256=attempt_payload.get("input_sha256"),
        prompt_revision=attempt_payload.get("prompt_revision"),
