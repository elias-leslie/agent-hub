...
class ParallelExecutor:
    ...
    async def _execute_all(
        ...
        if timeout:
            ...
            return [
                ...
                else _exception_to_result(Exception(str(r)), parent_id, trace_id)
                for r in raw_results
            ]
        return await asyncio.gather(*coros)

    def _create_timeout_result(
        self, results: list[SubagentResult], started_at: datetime, trace_id: str | None, span: Any
    ) -> ParallelResult:
    ...
    def _create_result(
        ...
        span.set_status(
            ...
            if status == "all_completed"
                ...
                StatusCode.ERROR,
                "Partial completion" if status == "partial" else "All tasks failed",
            )
        )

        logger.info(
            f"Parallel execution complete: {completed_count}/{total_tasks} succeeded, tokens: {total_input}+{total_output}"
        )
    ...
    async def execute(
        ...
        with tracer.start_as_current_span(
            ...
            try:
                if fail_fast:
                    results = await self._execute_fail_fast(
                        coros, overall_timeout, parent_id, effective_trace_id
                    )
                else:
                    results = await self._execute_all(
                        coros, overall_timeout, parent_id, effective_trace_id
                    )
            except TimeoutError:
