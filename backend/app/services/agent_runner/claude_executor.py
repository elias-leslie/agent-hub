...
from .executor_utils import load_session_history, save_sdk_session_id
...
def _track_response(
    ...
    response: CompletionInternalResult | CompletionResult,
) -> str | None:
...
async def _report_progress(
    result: AgentResult, progress: AgentProgress, callback: Any | None
) -> None:
...
async def _handle_finish(
    ...
    if finish_reason == "end_turn":
        ...
        await _report_progress(
            result,
            AgentProgress(turn=turn, status="complete", message="Agent completed task"),
            callback,
        )
        return True
    ...
    if finish_reason == "max_tokens":
        result.status, result.error, result.content, result.cited_uuids = (
            "error",
...
async def run_claude_code_execution(
    ...
    For Claude, this uses the SDK's native resume parameter.
    """
    model = config.model or CLAUDE_SONNET
    container_id = (
        config.container_id
        if config.container_id and container_manager.get(config.container_id)
        else None
    )
    all_cited_uuids: set[str] = set()
    session_id: str | None = None
    sdk_session_id: str | None = None  # Claude SDK session ID for resume
    turn = 0

    # Handle session continuation
    if config.resume_session_id and db is not None and isinstance(db, AsyncSession):
        try:
            history_messages, sdk_session_id = await load_session_history(
                db, config.resume_session_id, target_provider="claude"
            )
    ...
    while turn < config.max_turns:
        ...
        await _report_progress(
            result,
            AgentProgress(turn=turn, status="running", message=f"Turn {turn}: sending to Claude"),
            progress_callback,
        )

        try:
            response, new_session_id = await _get_completion(
                turn, db, session_id, messages, config, model, container_id, adapter
            )
            ...
            if turn == 1 and isinstance(response, CompletionInternalResult):
                ...
                # Save SDK session ID for future resume if we have one from response
                # (Currently complete_internal doesn't return SDK session, but this
                # prepares for when it does or when we use complete_with_tools)
                if (
                    sdk_session_id
                    and session_id
                    and db is not None
                    ...
                    await save_sdk_session_id(db, session_id, sdk_session_id)
            ...
            if turn > 1 and content and (prefixes := extract_uuid_prefixes(content)):
                all_cited_uuids.update(
                    (
                        await resolve_full_uuids(
                            prefixes, config.memory_group_id or config.project_id
                        )
                    ).values()
                )

            if await _handle_finish(
                finish_reason,
                turn,
                content,
                tool_calls,
