"""Entity browser API endpoints for auditing Graphiti-extracted entities."""
...
async def list_entities(
    group_id: Annotated[str, Query(description="Graphiti group_id to filter entities")],
    ...
    try:
        return await svc_list(graphiti.driver, group_id, search=search, limit=limit, offset=offset)
...
async def get_entity_health(
    group_id: Annotated[str, Query(description="Graphiti group_id")],
) -> Any:
    ...
    graphiti = get_graphiti()
    try:
        return await svc_health(graphiti.driver, group_id)
    except Exception as e:
...
async def get_entity_episodes(
    ...
    graphiti = get_graphiti()
    try:
        return await svc_episodes(graphiti.driver, entity_name, group_id, limit=limit)
    except Exception as e:
        raise HTTPException(
