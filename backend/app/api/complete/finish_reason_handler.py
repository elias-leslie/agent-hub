...
async def _handle_end_turn_persona_audit(
    turn: int,
    max_turns: int,
    result: Any,
    messages_for_adapter: list[Message],
    ...
    """Inject one-shot closeout audit for persona heartbeat/wake tasks. Returns None if not applicable."""
    if not (
        agent_slug == "persona"
    ...
    append_closeout_turn(messages_for_adapter, messages_dict, result.content, _CLOSEOUT_AUDIT_MSG)
...
async def _handle_end_turn(
    ...
    """Handle end_turn finish reason (after persona audit check)."""
    ...
    if closeout_plan.action == "recover":
        ...
        append_closeout_turn(
            messages_for_adapter, messages_dict, result.content, closeout_plan.prompt or ""
        )
...
async def handle_finish_reason(
    finish_reason: str | None, turn: int, max_turns: int, result: Any,
    messages_for_adapter: list[Message], messages_dict: list[dict[str, Any]],
    progress_log: list[AgentProgress], progress_callback: Callable[[AgentProgress], Any] | None,
    state: dict[str, Any], agent_slug: str | None, task_type: str | None,
    ...
    if finish_reason == "end_turn":
        audit_result = await _handle_end_turn_persona_audit(
            turn, max_turns, result, messages_for_adapter, messages_dict,
            progress_log, progress_callback, state, agent_slug, task_type,
        )
        if audit_result is not None:
            return audit_result
        return await _handle_end_turn(
            turn, cap, result, messages_for_adapter, messages_dict,
            progress_log, progress_callback, state,
        )

    if finish_reason == "tool_use":
        tool_calls_count = len(result.tool_calls or [])
        progress = create_progress(
            turn, "tool_use_requested",
