import type {
  MetricsResponse,
  RequestLogResponse,
} from '@/app/monitoring/requests/types'
...
export async function fetchRequestLog(params: {
  ...
  const response = await fetchApi(
    buildApiUrl(`/api/access-control/request-log?${searchParams.toString()}`),
  )
  if (!response.ok) {
...
export async function fetchMonitoringMetrics(
  ...
  const response = await fetchApi(
    buildApiUrl(`/api/access-control/metrics?hours=${hours}&limit=10`),
  )
  if (!response.ok) {
