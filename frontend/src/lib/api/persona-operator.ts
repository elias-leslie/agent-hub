...
export async function runPersonaWorkflow(request: WorkflowRequest): Promise<WorkflowResult> {
...
export async function fetchPersonaOperatorPreview(options: {
  ...
  const response = await fetchApi(`/api/agents/persona/preview?${params.toString()}`);
  if (!response.ok) {
...
export async function triggerPersonaAutomation(jobId: string): Promise<PersonaAutomationTriggerResponse> {
