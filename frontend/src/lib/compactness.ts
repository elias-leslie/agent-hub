...
export function analyzeCompactness(
  ...
  if (EXAMPLE_PATTERN.test(content)) {
    errors.push("example markers found. Strip examples; keep direct rules only.");
  }

  if (HEDGE_PATTERN.test(content)) {
    errors.push("hedging found. Replace maybe/should/could-style phrasing with direct rules.");
  }

  if (SOFT_TONE_PATTERN.test(content)) {
    errors.push("soft-tone phrasing found. Replace 'be X' guidance with direct action rules.");
  }

  if (OFFER_BACK_PATTERN.test(content)) {
    errors.push("offer-back phrasing found. Remove optional follow-up or helper language.");
  }
  ...
  if (sentenceLengths.some((count) => count > 24)) {
    errors.push("long prose sentences found. Split into short direct lines or bullets.");
  }
