...
const mockRuntimeState: {
  ...
  activePersonaSessions: Array<{
    id: string
    status?: string
    live_activity?: { status?: string; phase?: string }
  }>
  activeChildSessions: Array<{
    id: string
    status?: string
    live_activity?: { status?: string; phase?: string }
  }>
  ...
  activePersonaSessions: [
    {
      id: 'hb-1',
      status: 'active',
      live_activity: { status: 'active', phase: 'running_tool' },
    },
  ],
  activeChildSessions: [
    {
      id: 'child-1',
      status: 'active',
      live_activity: { status: 'active', phase: 'waiting_for_model' },
    },
  ],
  loading: false,
  error: null,
...
vi.mock('@/app/persona/components/UnifiedPersonaWorkspace', () => ({
  UnifiedPersonaWorkspace: ({
    ...
    <div data-testid="unified-workspace">
      workspace:{activeSessionId ?? 'new'}
    </div>
  ),
}))
...
describe('PersonaPage', () => {
  ...
  it('renders a compact operator header with the focused summary and core controls', () => {
    ...
    expect(screen.getByTestId('unified-workspace')).toHaveTextContent(
      'workspace:sess-123',
    )
