...
describe("persona settings tabs", () => {
  ...
  it("resets onboarding and refreshes persona state", async () => {
    ...
    vi.mocked(fetchApi).mockResolvedValueOnce(
      jsonResponse({ ...basePersona, onboarding_phase: "not_started", onboarding_complete: false }),
    );
    ...
    expect(fetchApi).toHaveBeenCalledWith("/api/persona/reset-onboarding", { method: "POST" });
    expect(onPersonaRefresh).toHaveBeenCalledWith(
      expect.objectContaining({ onboarding_phase: "not_started", onboarding_complete: false }),
    );
  });
  ...
  it("updates heartbeat interval and voice selection", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Heartbeat interval"), { target: { value: "120" } });
  ...
  it("filters voices and previews the selected sample", () => {
    render(<VoiceHeartbeatTab persona={basePersona} onUpdate={vi.fn()} autosave={idleAutosave} />);
  ...
  it("updates session reset mode, timing, and custom limits", () => {
    ...
    const { rerender } = render(
      <SessionLimitsTab
        persona={{ ...basePersona, session_reset_mode: "daily", limits: { max_turns: 300 } }}
        onUpdate={onUpdate}
      />,
    );

    fireEvent.change(screen.getByLabelText("Session reset hour"), { target: { value: "11" } });
    ...
    fireEvent.change(screen.getByLabelText("Max turns"), { target: { value: "350" } });
