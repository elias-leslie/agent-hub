...
describe('WorkspaceToolbar', () => {
  it('wraps mobile controls instead of forcing the range and filter buttons offscreen', () => {
    ...
    expect(screen.getByTestId('workspace-toolbar-controls')).toHaveClass(
      'flex-wrap',
      'sm:flex-nowrap',
    )
    expect(
      screen.getByPlaceholderText('Search history, tasks, files, agents...')
        .parentElement,
    ).toHaveClass('basis-full', 'sm:basis-auto')
    expect(
      screen.getByRole('button', { name: /24h/i }).parentElement,
