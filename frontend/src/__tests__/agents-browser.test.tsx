...
describe("agent browser UI", () => {
  it("uses the chat route from the row actions menu", () => {
    render(<AgentActionsMenu agent={baseAgent} onClone={vi.fn()} onArchive={vi.fn()} />);

    fireEvent.click(screen.getByRole("button", { name: /open actions for builder/i }));
  ...
  it("explains how to recover from an empty filtered state", () => {
    ...
    expect(screen.getByRole("button", { name: "Include inactive agents" })).toBeInTheDocument();
  });
});
