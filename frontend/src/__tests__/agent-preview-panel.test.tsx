...
describe("AgentPreviewPanel", () => {
  it("renders scenario controls and memory debug details", () => {
    ...
    expect(screen.getByText("Heartbeat preview builds the task prompt from live heartbeat inputs; only project and phase hints matter here.")).toBeInTheDocument();
  ...
  it("warns when chat preview would use an empty memory query", () => {
    ...
    expect(screen.getByText("Project scope is blank, so memory preview is using global-only scope.")).toBeInTheDocument();
    expect(screen.getByText("Chat preview has no user message yet, so the memory query will stay empty.")).toBeInTheDocument();
