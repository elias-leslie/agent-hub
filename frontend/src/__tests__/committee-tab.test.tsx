...
describe("committee tab", () => {
  ...
  it("renders default committee seats and persists strategy updates", () => {
    ...
    render(
      <CommitteeTab formData={formData} updateField={updateField} availableModels={models} />,
    );
      ...
      expect.objectContaining({
        committee: expect.objectContaining({
          seats: expect.arrayContaining([
            expect.objectContaining({ key: "macro", model_id: "claude-opus-4-7" }),
          ]),
        }),
      }),
    );
  ...
  it("runs a validation roundtable and renders the returned headline", async () => {
    ...
    render(
      <CommitteeTab formData={formData} updateField={vi.fn()} availableModels={models} />,
    );

    fireEvent.click(screen.getByRole("button", { name: /run validation roundtable/i }));

    await waitFor(() => {
      expect(fetchApiMock).toHaveBeenCalled();
    });
    expect(screen.getAllByText(/constructive risk appetite/i).length).toBeGreaterThan(0);
  });
});
