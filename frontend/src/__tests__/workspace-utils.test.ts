...
describe("workspace-utils prettifyDisplayText", () => {
  it("humanizes wrapped task payloads before display", () => {
    ...
    expect(formatted).toContain("Task task-605a52fc · pending · P2 · task · STANDARD");
    expect(formatted).toContain("Title: Live validation: Persona dispatch judgment after workflow cleanup");
    expect(formatted).toContain("Description: Temporary validation task to confirm the persona still reaches a clear readiness judgment after the latest workflow fixes.");
