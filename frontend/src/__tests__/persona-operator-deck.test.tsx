...
function renderDeck(overrides?: Partial<ComponentProps<typeof PersonaOperatorDeck>>) {
  const runtimeOverrides = overrides?.runtime ? (overrides.runtime as unknown as Partial<PersonaRuntimeState>) : undefined;
  ...
  return render(
    <PersonaOperatorDeck
      ...
      personaName="Avery"
      runtime={runtime}
      focusSession={(overrides?.focusSession as never) ?? runtime.primarySession}
      focusSessionDetails={(overrides?.focusSessionDetails as never) ?? runtime.primarySessionDetails}
      entries={overrides?.entries ?? []}
      pulse={(overrides?.pulse as never) ?? { metrics: [], issue_groups: [], agent_scorecards: [] }}
      visiblePulseMetrics={overrides?.visiblePulseMetrics ?? []}
      activeSessionId={overrides?.activeSessionId ?? runtime.primarySession?.id ?? null}
