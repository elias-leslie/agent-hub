...
describe('UnifiedPersonaWorkspace chat state', () => {
  ...
  it('prefers persisted completed session truth over a stale draft shell', async () => {
    mockUseChatStream.mockReturnValue({
      messages: [
        ...
        },
        {
          ...
          role: 'assistant',
          content:
            'Tool run complete. Highest-signal issue: stale session state.',
    ...
    expect(
      screen.queryByText('Waiting for model response'),
  ...
  it('overlays the footer on short viewports so chat space stays alive', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('persona-workspace-root')).toHaveAttribute(
        'data-compact-viewport',
        'true',
      )
    })
    expect(screen.getByTestId('persona-footer-shell').className).toContain(
      'absolute',
    )
  ...
  it('strips observability tags from assistant transcript bubbles', async () => {
      ...
      messages: [
        {
          ...
          role: 'assistant',
          content:
            '[[P:found]] Clean summary for operator view. Applied: [M:1234abcd] [[S:completed:clean summary]]',
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Clean summary for operator view.'),
      ).toBeInTheDocument()
    })
  ...
  it('prefers persisted session stop over draft cancel when both exist', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getAllByRole('button', { name: /Stop active work/i }).length,
      ).toBeGreaterThan(0)
    })

    fireEvent.click(
      screen.getAllByRole('button', { name: /Stop active work/i })[0],
    )
