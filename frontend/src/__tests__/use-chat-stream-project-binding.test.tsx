...
vi.mock(
  '../../../packages/chat-ui/src/hooks/chat-stream/session-loader',
  () => ({
...
describe('useChatStream project binding', () => {
          ...
          messages: [
            {
              ...
              timestamp: new Date('2026-04-16T00:00:00Z'),
            },
          ],
          projectId: 'agent-hub',
        }
          ...
          messages: [
            {
              ...
              timestamp: new Date('2026-04-16T00:01:00Z'),
            },
          ],
          projectId: 'summitflow',
        }
  ...
  it('retries the selected session load after a session-override preload failure', async () => {
    ...
    await act(async () => {
      ...
      rerender({ currentSessionId: 'sess-child' })
      pendingSend = sendFromCurrentThread(
        'Continue the summitflow lane.',
        undefined,
        'sess-child',
      )
    })
  ...
  it('preloads the target lane before sending a session override', async () => {
    ...
    await act(async () => {
      ...
      await result.current.sendMessage(
        'Redirect current work: finish the blocker review.',
        undefined,
        'sess-child',
      )
    })
