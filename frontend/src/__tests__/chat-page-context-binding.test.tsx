...
vi.mock('@/lib/api', () => ({
  fetchProjectPermissions: (...args: unknown[]) =>
    mocks.fetchProjectPermissions(...args),
...
describe('ChatPage context binding', () => {
      ...
      tasks: [
        {
          ...
          description:
            'No checkpoint closeout prints stale subtask branch noise.',
          status: 'pending',
          priority: 2,
          task_type: 'bug',
        },
  ...
  it('starts a fresh thread when project, agent, or task context changes', async () => {
    ...
    fireEvent.click(
      await screen.findByRole('button', { name: /Summitflow\s*summitflow/i }),
    )
    ...
    fireEvent.click(
      await screen.findByRole('button', { name: /task-6b0493a5/i }),
    )
  ...
  it('restores the active session independently for each agent context', async () => {
