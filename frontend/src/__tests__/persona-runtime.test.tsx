...
describe('usePersonaRuntime', () => {
  ...
  beforeEach(() => {
    ...
    mockCancelSessionStream.mockResolvedValue({
      cancelled: true,
      session_id: 'sess-1',
    })
  ...
  it('keeps preferred completed session details after active lists empty out', async () => {
    ...
    expect(result.current.primarySessionDetails?.live_activity?.summary).toBe(
      'Done',
    )
  })
  ...
  it('keeps active child sessions visible under a selected completed root thread', async () => {
    ...
    expect(result.current.activeChildSessions[0]?.parent_session_id).toBe(
      'sess-done',
    )
  })
})
