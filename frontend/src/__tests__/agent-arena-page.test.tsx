...
vi.mock("@/app/agents/[slug]/analytics/components/BenchmarkExperimentSection", () => ({
  BenchmarkExperimentSection: ({ experiments }: { experiments: Array<{ experiment_key: string }> }) => (
    <div>{experiments.length ? `experiment ${experiments[0].experiment_key}` : "no experiments"}</div>
  ),
}));
...
describe("AgentArenaDashboard", () => {
  ...
  it("shows experiments inside Arena instead of a separate analytics page", async () => {
    ...
    expect(screen.getByText("experiment persona-patience-ab")).toBeInTheDocument();
