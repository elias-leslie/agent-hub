...
describe("ProviderCard", () => {
  it("confirms OAuth disconnect inline before deleting credentials", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Confirm disconnect OAuth" }));
  ...
  it("confirms single credential deletion inline", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Delete credential sk-****1234" }));

    expect(screen.getByText("Delete key?")).toBeInTheDocument();
    expect(onDeleteCredential).not.toHaveBeenCalled();

    fireEvent.click(screen.getByRole("button", { name: "Confirm delete credential sk-****1234" }));
