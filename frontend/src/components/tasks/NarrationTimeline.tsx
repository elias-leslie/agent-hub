...
export function NarrationTimeline({ taskId }: { taskId: string }) {
  ...
  if (loading) return <div className="text-xs text-muted-foreground">Loading narration...</div>;
      ...
      <div className="space-y-0.5">
        {tags.map((tag) => (
          <div key={tag.id} className="flex items-start gap-2 text-xs">
            ...
            <span className={`font-mono ${TAG_COLORS[tag.tag_type] || "text-muted-foreground"}`}>
              {tag.tag_type}
            </span>
