...
function ProviderHealthPanel() {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {providers.map((p) => (
        <div key={p.provider} className="p-3 rounded-lg bg-slate-900/50 border border-slate-800">
          <div className="flex items-center justify-between mb-2">
            <p className="text-[11px] font-semibold text-slate-300 capitalize">{p.provider}</p>
            <span className={cn("px-1.5 py-0.5 rounded-full text-[8px] font-bold uppercase", stateColor(p.state))}>
              {p.state}
            </span>
          </div>
          <div className="grid grid-cols-2 gap-2 text-center">
            <div>
              <p className="text-xs font-mono font-bold text-slate-200">{formatLatency(p.latency_ms)}</p>
              <p className="text-[8px] text-slate-500 uppercase">Latency</p>
            </div>
            <div>
              <p className="text-xs font-mono font-bold text-slate-200">{p.availability.toFixed(1)}%</p>
              <p className="text-[8px] text-slate-500 uppercase">Avail</p>
            </div>
          </div>
          {p.last_error && (
            <p className="text-[9px] text-red-400/70 mt-2 truncate" title={p.last_error}>{p.last_error}</p>
          )}
        </div>
      ))}
    </div>
