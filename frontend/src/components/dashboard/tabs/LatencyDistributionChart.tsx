...
export function LatencyDistributionChart() {
  ...
  if (loading) return <div className="h-64 bg-slate-800 rounded animate-pulse" />;
    ...
    <div className="p-4 rounded-lg bg-slate-800/40 border border-slate-700/50">
      ...
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={chartData} layout="vertical" margin={{ left: 80, right: 20, top: 5, bottom: 5 }}>
          <XAxis type="number" tick={{ fill: "#94a3b8", fontSize: 10 }} tickFormatter={(v) => `${(v / 1000).toFixed(1)}s`} />
          <YAxis type="category" dataKey="model" tick={{ fill: "#cbd5e1", fontSize: 10 }} width={80} />
          <Tooltip
