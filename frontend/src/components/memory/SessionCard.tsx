...
const SOURCE_STYLES: Record<string, { label: string; bg: string; text: string }> = {
  ...
  image_generation: { label: "Image Gen", bg: "bg-cyan-500/15", text: "text-cyan-400" },
};

function getSourceStyle(source: string | null) {
  if (!source) return { label: "Unknown", bg: "bg-slate-500/15", text: "text-slate-400" };
  return SOURCE_STYLES[source] || { label: source, bg: "bg-slate-500/15", text: "text-slate-400" };
}
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-2">
              ...
              <span className={cn("px-2 py-0.5 rounded-full text-xs font-medium", sourceStyle.bg, sourceStyle.text)}>
                {sourceStyle.label}
              </span>
            </div>
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            <div className="flex items-center gap-3 text-xs">
              <span className="flex items-center gap-1 text-slate-300" title="Episodes loaded">
                <Eye className="h-3.5 w-3.5 text-slate-400" />
                {session.episodes_loaded}
              </span>
              <span className="flex items-center gap-1 text-slate-300" title="Episodes cited">
                <MessageCircle className="h-3.5 w-3.5 text-slate-400" />
                {session.episodes_cited}
              </span>
              <span
                className={cn("font-medium tabular-nums", getCitationColor(session.citation_rate))}
                title="Citation rate"
              >
        ...
        <div className="px-4 pb-4 border-t border-slate-800 pt-3">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="rounded-md bg-slate-800/60 p-3">
              ...
              <div className="text-xs text-slate-300 font-mono truncate">{session.session_id}</div>
            </div>
            <div className="rounded-md bg-slate-800/60 p-3">
              <div className="text-xs text-slate-500 mb-1">Episodes Loaded</div>
              <div className="text-lg font-semibold text-slate-100">{session.episodes_loaded}</div>
            </div>
            <div className="rounded-md bg-slate-800/60 p-3">
              <div className="text-xs text-slate-500 mb-1">Episodes Cited</div>
              <div className="text-lg font-semibold text-slate-100">{session.episodes_cited}</div>
            </div>
            <div className="rounded-md bg-slate-800/60 p-3">
              <div className="text-xs text-slate-500 mb-1">Citation Rate</div>
              <div className={cn("text-lg font-semibold", getCitationColor(session.citation_rate))}>
