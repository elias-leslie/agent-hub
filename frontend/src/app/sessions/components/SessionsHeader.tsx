        ...
        <div className="flex flex-col gap-2.5 py-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="min-w-0 space-y-1.5">
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-base font-semibold tracking-tight text-slate-100">Sessions</h1>
            </div>
            <div className="flex flex-wrap items-center gap-2 text-[13px] text-slate-400">
              <span data-testid="sessions-filter-scope" className="inline-flex flex-wrap items-center gap-1.5">
                <span className="text-slate-300">
                  <span data-testid="sessions-visible-count">{formattedVisible}</span> visible
                </span>
                <span className="text-slate-700">·</span>
                <span>
                  <span data-testid="sessions-loaded-count">{formattedLoaded}</span> loaded
                </span>
                <span className="text-slate-700">·</span>
                <span>
                  <span data-testid="sessions-total-count">{formattedTotal}</span> total
                </span>
              </span>
              {modelFilter ? <ModelFilterBadge modelFilter={modelFilter} onClear={onClearModelFilter} /> : null}
            </div>
          </div>
