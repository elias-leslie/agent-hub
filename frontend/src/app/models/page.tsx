...
import {
  type ModelOption,
  useModelsWithSync,
} from '@/components/chat/use-models'
...
export default function ModelsPage() {
  ...
  const effectiveSelectedProviders =
    selectedProviders ?? new Set(availableProviderIds)
  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        'Sync failed',
        'Could not refresh external benchmark and pricing overlays',
      )
    } finally {
      setSyncing(false)
    }
  ...
  const filteredModels = useMemo(() => {
    ...
    filtered = filtered.filter((model) =>
      effectiveSelectedProviders.has(model.provider),
    )
    ...
    if (capabilityFilters.imageGen)
      filtered = filtered.filter(
        (model) => model.capabilities.can_generate_images,
      )
  ...
  const summaryCards = [
    ...
    },
    {
      label: 'Watchlist',
      value: compactNumber(
        catalogHealth?.discovery?.unmatched_model_count ?? 0,
      ),
              ...
              <div className="max-w-3xl space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <p className="max-w-2xl text-sm leading-6 text-slate-300">
                    Curated registry stays authoritative. GPT-5.5 is routed
                    through Codex OAuth until API availability lands; sync
                    overlays live pricing and benchmark evidence for API-backed
                    models.
                  </p>
                </div>
              </div>
              ...
              <section className="rounded-[24px] border border-slate-800/80 bg-slate-950/70 p-5">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h2 className="mt-2 text-lg font-semibold text-slate-100">
                      External candidates not in catalog
                    </h2>
                  </div>
                      ...
                      >
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          {provider.sample_model_ids.map((sample) => (
                            <span
                              key={sample}
                              className="rounded-full border border-slate-800 bg-slate-950 px-2.5 py-1 font-mono text-[11px] text-slate-300"
                            >
                              {sample}
                            </span>
                          ))}
          ...
          <div className="mt-6 flex items-center justify-center py-16 text-center">
            <div className="max-w-md rounded-[24px] border border-rose-900/60 bg-rose-950/20 p-8">
              ...
              <p className="mt-2 text-sm text-slate-400">
                Models API did not return catalog data. Retry after backend
                check.
              </p>
              <button
                type="button"
        ...
        {filteredModels.length === 0 && !isError && !isLoading && (
          <div className="mt-6 flex flex-col items-center justify-center rounded-[24px] border border-slate-800 bg-slate-950/80 py-16 text-center">
            ...
            <p className="mt-2 max-w-md text-sm text-slate-400">
              Current filters excluded every catalog entry. Change provider,
              capability, or search.
            </p>
          </div>
        )}
