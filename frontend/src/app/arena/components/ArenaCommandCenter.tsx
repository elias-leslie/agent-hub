...
function deriveSystemStatus(overview: ArenaOverview, personaName: string) {
  if (
    overview.system.total_regressions >= 8 ||
    (overview.system.avg_pass_rate ?? 100) < 60
  ) {
    return {
  ...
  if (
    ...
    return {
      ...
      detail:
        'The loop is running, but there are still signal-quality or reliability gaps worth tightening.',
    }
  }
  return {
    label: 'Stable',
    ...
    detail:
      'Autonomy, memory, and benchmark signals are currently aligned without obvious pressure spikes.',
  }
}

function derivePressureCount(
  agents: Array<{ benchmark: ArenaAgentBenchmarkSummary }>,
) {
...
function SkeletonCard({ className }: { className: string }) {
  return (
    <div
      className={`rounded-xl bg-slate-800/70 animate-shimmer ${className}`}
    />
  )
}
    ...
    () =>
      overview
        ? getScheduledJob(
            ...
            (job) => job.payload_type === 'agent_turn',
          )
        : null,
    [overview],
  )
  const honingJob = useMemo(
    () =>
      overview
        ? getScheduledJob(
            overview.scheduled_jobs,
            (job) => job.payload_type === 'self_honing',
          )
        : null,
    [overview],
  )
      ...
      <main className="relative mx-auto max-w-7xl p-6 lg:p-8">
            ...
            <div className="max-w-3xl">
              ...
              <h2 className="mt-4 text-3xl font-semibold tracking-tight lg:text-4xl">
                <span className="gradient-text-amber">
                  See autonomy, memory, and benchmark
                </span>
                <br />
                <span className="text-slate-50">pressure in one place.</span>
              </h2>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">
                Arena now tracks the operational loop itself: scheduled review,
                scheduled honing, memory signal quality, and the agents
                currently producing the most pressure.
              </p>
        ...
        <section className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div className="group rounded-2xl border border-slate-800 bg-slate-900/60 p-5 transition-colors hover:border-emerald-900/60 animate-fade-up stagger-1">
            ...
            <p className="mt-3 text-3xl font-semibold text-slate-50">
              {overview.system.agents_with_history}
              <span className="text-lg text-slate-500">
                /{overview.system.total_agents}
              </span>
            </p>
          ...
          <div className="group rounded-2xl border border-slate-800 bg-slate-900/60 p-5 transition-colors hover:border-blue-900/60 animate-fade-up stagger-2">
            ...
            <p className="mt-3 text-3xl font-semibold text-slate-50">
              {overview.system.total_runs}
            </p>
            <p className="mt-2 text-sm text-slate-400">
              In current {windowDays}-day window
            </p>
          </div>
          ...
          <div className="group rounded-2xl border border-slate-800 bg-slate-900/60 p-5 transition-colors hover:border-rose-900/60 animate-fade-up stagger-4">
            <div className="flex items-center justify-between">
              ...
              <div
                className={cn(
                  ...
                  overview.system.total_regressions > 0
                    ? 'bg-rose-500/80'
                    : 'bg-slate-600',
                )}
              />
            ...
            <p className="mt-3 text-3xl font-semibold text-slate-50">
              {overview.system.total_regressions}
            </p>
            {Object.keys(overview.system.regressions_by_category ?? {}).length >
              0 && (
        ...
        <section className="mt-4 rounded-2xl border border-amber-900/30 bg-amber-950/10 px-5 py-4">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-300">
            Arena is a triage surface, not a single blended score. The average
            covers
            {` ${overview.system.agents_with_history} of ${overview.system.total_agents} `}
            agents in this window. The regression count tracks decision-quality
            misses only — JSON/summary-term failures and tool-requirement misses
            are shown as labeled pulse items below.
          </p>
        </section>
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  {personaPossessive} evidence review loop for small justified
                  changes.
                </p>
              </div>
            ...
            <dl className="mt-4 space-y-3 text-sm">
              <div className="flex items-center justify-between">
                ...
                <dd className="font-semibold text-slate-100">
                  {formatDateTime(reviewJob?.next_run_at)}
                </dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-slate-400">Last run</dt>
                <dd className="font-semibold text-slate-100">
                  {formatRelativeTime(reviewJob?.last_run_at)}
                </dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-slate-400">Run count</dt>
                <dd className="font-semibold text-slate-100">
                  {reviewJob?.run_count ?? 0}
                </dd>
              </div>
            </dl>
          </article>
          ...
          <article className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 transition-colors hover:border-violet-900/40">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  Benchmark-backed self-correction for {personaName} and{' '}
                  {personaPossessive} routing choices.
                </p>
              </div>
            ...
            <dl className="mt-4 space-y-3 text-sm">
              <div className="flex items-center justify-between">
                <dt className="text-slate-400">Next run</dt>
                <dd className="font-semibold text-slate-100">
                  {formatDateTime(honingJob?.next_run_at)}
                </dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-slate-400">Last run</dt>
                <dd className="font-semibold text-slate-100">
                  {formatRelativeTime(honingJob?.last_run_at)}
                </dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-slate-400">Run count</dt>
                <dd className="font-semibold text-slate-100">
                  {honingJob?.run_count ?? 0}
                </dd>
              </div>
            </dl>
          </article>
          ...
          <article className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 transition-colors hover:border-blue-900/40">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  Whether injected memory is getting cited, searched, and
                  debugged properly.
                </p>
              </div>
            ...
            <dl className="mt-4 space-y-3 text-sm">
                ...
                <dd className="font-semibold text-slate-100">
                  {formatPercent(
                    overview.memory_utilization
                      .selected_reference_citation_rate * 100,
                  )}
                </dd>
              </div>
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between">
                ...
                <dd className="font-semibold text-slate-100">
                  {formatPercent(
                    overview.memory_utilization.memory_debug_coverage_rate *
                      100,
                  )}
                </dd>
              </div>
          ...
          <article className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  Latest negative signals that are actually worth{' '}
                  {personaPossessive} attention.
                </p>
              </div>
            ...
            <div className="mt-4 grid gap-3">
              {overview.repeated_issues.length > 0 ? (
                overview.repeated_issues.slice(0, 4).map((issue) => (
                  <article
                    key={`${issue.agent_slug}:${issue.content}`}
                  ...
                  >
                    ...
                    <p className="mt-2 text-[11px] text-slate-500">
                      Latest {formatDateTime(issue.latest_at)}
                    </p>
                  </article>
                ))
              ) : (
                <div className="rounded-xl border border-dashed border-emerald-900 bg-emerald-950/20 px-4 py-6 text-center text-sm text-slate-300">
                  No negative issue signals surfaced in the current evidence
                  window.
                </div>
              )}
            </div>
          ...
          <div className="grid gap-6">
            <article className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-400">
                    References that are selected often enough to cost context
                    but rarely cited.
                  </p>
                </div>
              ...
              <div className="mt-4 space-y-3">
                {overview.low_yield_references.length > 0 ? (
                  overview.low_yield_references.slice(0, 4).map((item) => (
                    <article
                      key={item.uuid}
                      className="rounded-xl border border-slate-800 bg-slate-950/50 px-4 py-3"
                    >
                      ...
                      <p className="mt-1 text-xs text-slate-400">
                        selected {item.selected} · cited {item.cited} · rate{' '}
                        {formatPercent(item.citation_rate * 100)}
                      </p>
                      {item.tags.length > 0 ? (
                        <p className="mt-2 text-[11px] text-slate-500">
                          {item.tags.join(', ')}
            ...
            <article className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-400">
                    Structural context-shape issues: broad references, bloated
                    policy, and trigger drift.
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <span
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="rounded-xl border border-slate-800 bg-slate-950/50 px-4 py-3 text-sm text-slate-300">
                  ...
                  <p className="mt-2 text-slate-400">
                    ...
                    {' · '}
                    threshold breaches{' '}
                    {memoryGovernance?.soft_limit_breach_count ?? 0}
                  </p>
                  <p className="mt-2">
                    custom configs{' '}
                    {memoryGovernance?.custom_memory_config_agent_count ?? 0} /{' '}
                    {memoryGovernance?.active_agent_count ?? 0}
                  </p>
                  <p className="mt-1 text-slate-400">
                    tool ctx off{' '}
                    {memoryGovernance?.tool_capabilities_disabled_agent_count ??
                      0}
                    {' · '}
                    project index off{' '}
                    {memoryGovernance?.project_index_disabled_agent_count ?? 0}
                    {' · '}
                    ref index off{' '}
                    {memoryGovernance?.reference_index_disabled_agent_count ??
                      0}
                  </p>
                  <p className="mt-1 text-slate-400">
                    agents with UUID exclusions{' '}
                    {memoryGovernance?.memory_exclusion_agent_count ?? 0}
                    {' · '}
                    excluded UUIDs{' '}
                    {memoryGovernance?.excluded_memory_uuid_count ?? 0}
                  </p>
                </div>
                {memoryGovernance &&
                memoryGovernance.untargeted_reference_samples.length > 0 ? (
                  <div className="space-y-2">
                    <p className="text-xs uppercase tracking-wide text-slate-500">
                      Top untargeted references
                    </p>
                    {memoryGovernance.untargeted_reference_samples
                      .slice(0, 3)
                      .map((item) => (
                        <div
                          key={item.uuid}
                ...
                memoryGovernance.oversized_policy_samples.length > 0 ? (
                  <div className="space-y-2">
                    <p className="text-xs uppercase tracking-wide text-slate-500">
                      Top oversized policies
                    </p>
                    {memoryGovernance.oversized_policy_samples
                      .slice(0, 3)
                      .map((item) => (
                        <div
                          key={item.uuid}
                ...
                {memoryGovernance &&
                memoryGovernance.invalid_trigger_task_type_samples.length >
                  0 ? (
                  <div className="space-y-2">
                    <p className="text-xs uppercase tracking-wide text-slate-500">
                      Invalid trigger samples
                    </p>
                ...
                {memoryGovernance &&
                memoryGovernance.startup_profile_agent_target_samples.length >
                  0 ? (
                  <div className="space-y-2">
                    <p className="text-xs uppercase tracking-wide text-slate-500">
                      Broken startup routes
                    </p>
                ...
                memoryGovernance.startup_profile_agent_target_samples.length ===
                  0 ? (
                  <div className="rounded-xl border border-dashed border-emerald-900 bg-emerald-950/20 px-4 py-6 text-center text-sm text-slate-300">
                    No structural memory-governance issues surfaced in the
                    current snapshot.
                  </div>
                ) : null}
              </div>
            ...
            <article className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-sm font-semibold text-slate-100">
                    Experiment + regression pulse
                  </h3>
                  <p className="mt-1 text-sm text-slate-400">
                    Recent experiment decisions and open benchmark failures from{' '}
                    {personaPossessive} loop.
                  </p>
                </div>
                <Trophy className="h-5 w-5 text-amber-500" />
              </div>
              <div className="mt-4 space-y-3">
                {overview.recent_benchmark_experiments
                  .slice(0, 2)
                  .map((experiment) => (
                    <div
                      key={`${experiment.suite_id}:${experiment.decision}`}
                      className="rounded-xl border border-slate-800 bg-slate-950/50 px-4 py-3 text-sm"
                    >
                      <p className="font-semibold text-slate-100">
                        {experiment.suite_id}
                      ...
                      <p className="mt-1 text-xs text-slate-400">
                        decision {experiment.decision} · score{' '}
                        {experiment.score_delta.toFixed(1)} · pass{' '}
                        {experiment.pass_rate_delta.toFixed(1)}
                      </p>
                ...
                {overview.open_regression_clusters
                  .slice(0, 2)
                  .map((cluster) => (
                    <div
                      key={`${cluster.case_id}:${cluster.failure_detail}`}
                      className="rounded-xl border border-rose-900 bg-rose-950/20 px-4 py-3 text-sm"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <p className="font-semibold text-slate-100">
                          {cluster.case_id}
                ...
                {overview.recent_benchmark_experiments.length === 0 &&
                overview.open_regression_clusters.length === 0 ? (
                  <div className="rounded-xl border border-dashed border-emerald-900 bg-emerald-950/20 px-4 py-6 text-center text-sm text-slate-300">
          ...
          {overview.agents.length > 0 ? (
            <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
              {overview.agents.map((agent) => (
                <ArenaAgentCard key={agent.slug} agent={agent} />
              ))}
            </div>
          ) : (
