...
export function SessionHotspotsPanel({ hotspots, isLoading }: SessionHotspotsPanelProps) {
  return (
      ...
      {isLoading || !hotspots ? (
        ...
        <>
          <div className="grid gap-4 lg:grid-cols-4">
            <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-5">
              ...
              <p className="mt-2 text-xs text-slate-500">
                {formatTokens(hotspots.totals.input_tokens)} in / {formatTokens(hotspots.totals.output_tokens)} out
              </p>
            </div>
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-5">
              <h3 className="text-sm font-semibold text-slate-100">Attribution Breakdown</h3>
              <div className="mt-4 space-y-3">
                {hotspots.attribution_breakdown.length === 0 ? (
                  ...
                  <div key={row.kind} className="rounded-lg border border-slate-800 bg-slate-950/70 px-4 py-3">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-medium text-slate-100">{row.label}</p>
                        <p className="text-xs text-slate-500">
                          {row.sessions} sessions · {formatTokens(row.input_tokens)} in
                        </p>
                      </div>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-5">
              <h3 className="text-sm font-semibold text-slate-100">Repeated Workloads</h3>
              <div className="mt-4 space-y-3">
                {hotspots.repeated_workloads.length === 0 ? (
                  ...
                  <div key={row.workload_key} className="rounded-lg border border-slate-800 bg-slate-950/70 px-4 py-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-slate-100 truncate">{row.label}</p>
                        <p className="text-xs text-slate-500">
                          {row.project_id} · {row.agent_slug || "—"} · {row.sessions} sessions
                        </p>
                        {row.detail && (
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-5">
              <h3 className="text-sm font-semibold text-slate-100">Low-Yield Sessions</h3>
              <div className="mt-4 space-y-3">
                {hotspots.low_yield_sessions.length === 0 ? (
                  ...
                  <div key={row.session_id} className="rounded-lg border border-slate-800 bg-slate-950/70 px-4 py-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-slate-100 truncate">{row.label}</p>
                        <p className="text-xs text-slate-500">
                          {row.project_id} · {row.agent_slug || "—"} · {row.attribution_label || "Unknown"}
                        </p>
                        <p className="mt-1 text-[11px] text-slate-600">
                          {formatTokens(row.input_tokens)} in / {formatTokens(row.output_tokens)} out · ratio {row.efficiency_ratio}x
                        </p>
                      </div>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-5">
              <h3 className="text-sm font-semibold text-slate-100">Zero-Event Active Sessions</h3>
              <div className="mt-4 space-y-3">
                  ...
                  <div key={row.session_id} className="rounded-lg border border-slate-800 bg-slate-950/70 px-4 py-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-slate-500">
                          {row.request_source || "no request source"} · {row.lifecycle_state || "unknown"}
                        </p>
                      </div>
                      <span className="text-sm font-semibold text-rose-300">
