...
import { type ProjectBudget, type BudgetSettingsUpdate } from "@/lib/api";
...
export function BudgetEditor({
  ...
  const handleSave = () => {
    const update: BudgetSettingsUpdate = {
      ...
      budget_alert_threshold: alertThreshold ? parseFloat(alertThreshold) : null,
    };
    ...
    <td colSpan={5} className="px-4 py-3">
      <div className="flex items-center gap-6 bg-slate-800/30 rounded-lg p-3 border border-slate-700/50">
        <div className="flex items-center gap-2">
          <label className="text-xs text-slate-400 whitespace-nowrap">Daily limit ($)</label>
          <input
            type="number"
            step="0.01"
            min="0"
        ...
        <div className="flex items-center gap-2">
          <label className="text-xs text-slate-400 whitespace-nowrap">Monthly limit ($)</label>
          <input
            type="number"
            step="0.01"
            min="0"
        ...
        <div className="flex items-center gap-2">
          <label className="text-xs text-slate-400 whitespace-nowrap">Alert at (%)</label>
          <input
            type="number"
            step="0.05"
            min="0"
