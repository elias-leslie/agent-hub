...
import { ALERT_CONFIG, getProgressColor, getProgressBgColor, getProgressPercent } from "./_utils";

// ─── Alert level badge ────────────────────────────────────────────────────────

export function AlertBadge({ level }: { level: "warning" | "critical" | null }) {
  if (!level) {
    return (
...
export function BudgetBar({ used, limit }: { used: number; limit: number | null }) {
  ...
  if (limit === null) {
    return (
      <div className="flex items-center gap-2">
        <span className="text-sm font-mono text-slate-300">{formatCurrency(used)}</span>
  ...
  return (
    <div className="min-w-[160px]">
      <div className="flex items-baseline justify-between mb-1">
        <span className="text-sm font-mono text-slate-300">{formatCurrency(used)}</span>
        <span className="text-[11px] text-slate-500">/ {formatCurrency(limit)}</span>
      </div>
      ...
      <div className="mt-0.5 text-right">
        <span className="text-[10px] text-slate-600">{percent.toFixed(0)}%</span>
      </div>
    </div>
  );
}
  ...
  return (
    <span
      className={cn(
        ...
        "animate-pulse",
        error ? "bg-red-900/30 text-red-400" : "bg-emerald-900/30 text-emerald-400",
      )}
    >
      {error ? (
        <>
