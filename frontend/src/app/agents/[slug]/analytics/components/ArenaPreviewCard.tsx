...
export function ArenaPreviewCard({
  ...
  return (
    <ChartCard title="Arena">
      <div className="relative overflow-hidden rounded-2xl border border-cyan-200/70 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.14),transparent_34%),radial-gradient(circle_at_bottom_right,rgba(14,165,233,0.12),transparent_30%),linear-gradient(180deg,#ffffff_0%,#f8fbff_100%)] p-5 dark:border-cyan-900/40 dark:bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.16),transparent_34%),radial-gradient(circle_at_bottom_right,rgba(14,165,233,0.14),transparent_30%),linear-gradient(180deg,#0f172a_0%,#020617_100%)]">
        <div className="pointer-events-none absolute inset-0 opacity-30 [background-image:linear-gradient(to_right,rgba(148,163,184,0.14)_1px,transparent_1px),linear-gradient(to_bottom,rgba(148,163,184,0.14)_1px,transparent_1px)] [background-size:24px_24px]" />
        <div className="relative">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-cyan-200 dark:bg-cyan-950/60 dark:text-cyan-100">
          ...
          <div className={`mt-4 grid gap-3 ${compact ? "md:grid-cols-4" : "md:grid-cols-2 xl:grid-cols-4"}`}>
            <div className="rounded-xl bg-white/90 px-3 py-3 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900/90 dark:ring-slate-800">
              <p className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Runs</p>
              <p className="mt-1 text-xl font-semibold text-slate-950 dark:text-slate-50">
                {dashboard.overview.total_runs}
              </p>
            </div>
            <div className="rounded-xl bg-white/90 px-3 py-3 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900/90 dark:ring-slate-800">
              <p className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Avg score</p>
              <p className="mt-1 text-xl font-semibold text-slate-950 dark:text-slate-50">
                {formatScore(dashboard.overview.avg_score)}
              </p>
            </div>
            <div className="rounded-xl bg-white/90 px-3 py-3 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900/90 dark:ring-slate-800">
              <p className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Pass rate</p>
              <p className="mt-1 text-xl font-semibold text-slate-950 dark:text-slate-50">
                {formatPercent(dashboard.overview.pass_rate)}
              </p>
            </div>
            <div className="rounded-xl bg-white/90 px-3 py-3 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900/90 dark:ring-slate-800">
              <p className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Suites</p>
              <p className="mt-1 inline-flex items-center gap-2 text-xl font-semibold text-slate-950 dark:text-slate-50">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
            ...
            {ctaLabel ? (
              <Link
                href={slug === "persona" ? "/persona/arena" : `/agents/${slug}/arena`}
