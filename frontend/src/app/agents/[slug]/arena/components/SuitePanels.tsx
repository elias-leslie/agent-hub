...
import type { AgentBenchmarkCaseSummary, AgentBenchmarkSuiteSummary } from "@/app/agents/[slug]/analytics/types";
import { formatArenaLabel, formatPercent, formatRelativeTime, formatScore } from "@/app/arena/utils";
        ...
        {sortedSuites.length === 0 ? (
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {sortedSuites.map((suite) => {
              ...
              const tone = hasIssues ? "border-amber-900 bg-amber-950/20" : "border-emerald-900 bg-emerald-950/20";
              const badgeTone =
              ...
              return (
                <article key={suite.suite_id} className={`rounded-2xl border px-4 py-4 ${tone}`}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-slate-100">{formatArenaLabel(suite.suite_id)}</p>
                      <p className="mt-1 text-xs text-slate-300">
                        {suite.run_count} runs · {suite.case_ids.length} cases ·{" "}
                        {suite.run_kinds.join(", ") || "benchmark"}
                      </p>
                    </div>
                    <span className={`rounded-full px-2 py-1 text-[11px] font-semibold ring-1 ${badgeTone}`}>
                      {suite.open_regressions > 0 ? `${suite.open_regressions} regressions` : "stable"}
                    </span>
                  </div>
                  ...
                  <dl className="mt-4 grid grid-cols-3 gap-3 text-sm">
                    <div>
                      ...
                      <dd className="mt-1 font-semibold text-slate-100">{formatScore(suite.avg_score)}</dd>
                    </div>
                    <div>
                      <dt className="text-xs uppercase tracking-wide text-slate-400">Pass rate</dt>
                      <dd className="mt-1 font-semibold text-slate-100">{formatPercent(suite.pass_rate)}</dd>
                    </div>
                    <div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    {caseSummary.case_name ? (
                      <p className="mt-0.5 text-[11px] font-mono text-slate-500">{caseSummary.case_id}</p>
                    ) : null}
                    <p className="mt-1 text-xs text-slate-400">
                      {caseSummary.suite_ids.map((suiteId) => formatArenaLabel(suiteId)).join(", ")} ·{" "}
                      {caseSummary.attempts} attempts
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-semibold text-slate-50">{formatScore(caseSummary.avg_score)}</p>
                    <p className="text-xs text-slate-400">{formatPercent(caseSummary.pass_rate)}</p>
                  </div>
                </div>

                {caseSummary.latest_failure_detail ? (
                  <p className="mt-3 text-xs text-slate-300">{caseSummary.latest_failure_detail}</p>
                ) : null}
