    ...
    <ChartCard title="Regression watchlist">
      {regressions.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-emerald-900 bg-emerald-950/20 px-5 py-8 text-center">
          <p className="text-sm font-semibold text-slate-100">No open regression clusters</p>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-slate-100">{regression.case_id}</p>
                  <p className="mt-1 text-xs text-slate-300">{regression.failure_detail}</p>
                </div>
