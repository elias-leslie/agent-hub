...
export function OverviewPanels({
        ...
        <div className="mt-6">
          <ChartCard title="Arena evidence">
            <div className="rounded-2xl border border-dashed border-amber-900 bg-amber-950/20 px-6 py-10 text-center">
              <p className="text-sm font-semibold text-slate-100">Arena benchmark history unavailable</p>
  ...
  if (!hasHistory) {
    return (
      <>
        <div className="mt-6">
          <ArenaPreviewCard dashboard={benchmarkDashboard} slug={slug} ctaLabel={null} compact={false} />
        </div>
        <RuntimePanels
          analytics={analytics}
          primaryModel={primaryModel}
  ...
  return (
    <>
      <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
        <KPICard label="Benchmark Runs" value={benchmarkDashboard.overview.total_runs} icon={FlaskConical} color="blue" />
        <KPICard label="Average Score" value={formatScore(benchmarkDashboard.overview.avg_score)} icon={Trophy} color="green" />
        <KPICard label="Pass Rate" value={formatPercent(benchmarkDashboard.overview.pass_rate)} icon={Activity} color="amber" />
        <KPICard label="Open Regressions" value={benchmarkDashboard.overview.open_regressions} icon={AlertTriangle} color="red" />
      </div>
                ...
                return (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-sm font-semibold text-slate-100">{model.model_id}</p>
                    ...
                    <div className="text-right">
                      <p className="text-lg font-semibold text-slate-50">{formatScore(model.avg_score)}</p>
                      <p className="text-xs text-slate-400">pass {formatPercent(model.pass_rate)}</p>
                    </div>
                  </article>
                );
              })}
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <ChartCard title="Runtime pulse">
          {runtimeErrorMessage || !analytics ? (
            <p className="text-sm text-slate-400">{runtimeErrorMessage ?? "Runtime metrics are unavailable."}</p>
          ) : (
