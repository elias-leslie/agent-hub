...
function AttemptRow({ attempt }: { attempt: AgentBenchmarkAttemptDetail }) {
  return (
    ...
    >
      ...
      <span className="shrink-0 w-16 text-right text-xs text-slate-400">{attempt.latency_ms}ms</span>
    </div>
  );
}
          ...
          {failedAttempts.map((attempt) => (
            <div key={attempt.id}>
              ...
              {attempt.failure_detail ? (
                <p className="ml-5 mt-1 text-[11px] text-rose-400">{attempt.failure_detail}</p>
              ) : null}
            </div>
          ))}
        </>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-semibold text-slate-100">{formatArenaLabel(run.suite_id)}</p>
                    <p className="mt-1 text-xs uppercase tracking-[0.16em] text-slate-400">{run.run_kind}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-semibold text-slate-50">{formatScore(run.avg_score)}</p>
                    <p className="text-xs text-slate-400">{formatPercent(run.pass_rate)}</p>
                  </div>
                </div>

                <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <dt className="text-xs uppercase tracking-wide text-slate-400">Attempts</dt>
                    <dd className="mt-1 font-medium text-slate-100">
                      {run.passed_attempt_count}/{run.attempt_count}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs uppercase tracking-wide text-slate-400">Infra failures</dt>
                    <dd className="mt-1 font-medium text-slate-100">{run.infra_failure_count}</dd>
                  </div>
                </dl>

                <p className="mt-4 text-xs text-slate-400">Models: {run.models.join(", ")}</p>
                <p className="mt-1 text-xs text-slate-400">Cases: {run.case_ids.join(", ")}</p>
