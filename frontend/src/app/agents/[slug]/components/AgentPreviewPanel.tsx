...
function getPromptInputConfig(mode: PreviewTaskType) {
  ...
  if (mode === "review") {
    return {
      ...
      placeholder: "Paste the completion or outcome text you want the review prompt to inspect.",
  ...
  return {
    ...
    helper: "Used as the chat memory query so the memory plan matches a real request.",
  };
}

function buildWarnings(mode: PreviewTaskType, scenario: PreviewScenario): string[] {
  const warnings: string[] = [];
  if (!scenario.projectId.trim()) {
    warnings.push("Project scope is blank, so memory preview is using global-only scope.");
  }
  if (mode === "chat" && !scenario.promptInput.trim()) {
    warnings.push("Chat preview has no user message yet, so the memory query will stay empty.");
  }
  if (mode === "wake" && !scenario.promptInput.trim()) {
    warnings.push("Wake preview will fall back to the backend placeholder task until you add wake text.");
  }
  if (mode === "review" && !scenario.promptInput.trim()) {
    warnings.push("Review preview will use placeholder completion content until you provide it.");
  }
  if (mode === "review" && !scenario.phase.trim()) {
    warnings.push("Review preview will use placeholder workstream inventory until you provide a phase hint.");
  }
  return warnings;
}

function renderTokenSummary(entry: AgentPreviewMemoryPlanEntry): string {
  const rendered = typeof entry.rendered_tokens === "number" ? entry.rendered_tokens : null;
...
export function AgentPreviewPanel({
  ...
  return (
    <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-4 border-slate-800 dark:bg-slate-900">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className={cn(
                ...
                previewMode === mode.value
                  ? "border-slate-900 bg-slate-900 text-slate-900 bg-slate-100 dark:text-slate-900"
                  : "border-slate-200 text-slate-600 hover:border-slate-700 dark:text-slate-300",
              )}
            >
              {mode.label}
            </button>
          ...
          <button
            ...
            className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-700 transition border-slate-700 text-slate-200 dark:hover:bg-slate-800"
          >
      ...
      {previewMode !== "heartbeat" ? (
        ...
        <div className="rounded-lg border border-dashed border-slate-300 px-3 py-2 text-[11px] text-slate-500 border-slate-700 dark:text-slate-400">
          Heartbeat preview builds the task prompt from live heartbeat inputs; only project and phase hints matter here.
        </div>
      )}

      {warnings.length > 0 ? (
        <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-3 dark:border-amber-900/40 dark:bg-amber-950/20">
        ...
        <div className="rounded-xl bg-slate-950 p-4">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div className="text-[11px] text-slate-500">
              {previewFetching ? "Refreshing preview..." : "Preview updates automatically as the scenario changes."}
            </div>
          ...
          {previewFetching && !preview ? (
            ...
            <div className="space-y-4">
              <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
                <StatCard label="Sections" value={formatNumber(preview.sections.length)} />
                <StatCard label="Mandates" value={formatNumber(preview.mandate_count)} />
                ...
                <div className="rounded-lg border border-slate-800 bg-slate-900 p-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(tierCounts).length > 0 ? (
                      ...
                      <span className="text-xs text-slate-500">No tier data</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-lg border border-slate-800 bg-slate-900 p-3">
                ...
                {memoryPlan.length > 0 ? (
                  <div className="space-y-2">
                    {memoryPlan.map((entry) => (
                      ...
                      >
                        ...
                        <div className="text-[11px] font-medium text-slate-300">{entry.tier}</div>
                        <div className="text-[11px] text-slate-400">{entry.reason}</div>
                  ...
                  <p className="text-xs text-slate-500">No memory-plan details returned.</p>
                )}
              </div>
            ...
            <p className="py-8 text-center text-sm text-slate-500">Preview unavailable.</p>
          )}
        </div>
      ) : null}
    </div>
