...
import type { Agent, CommitteeConfig, CommitteeSeatConfig, ModelInfo } from "../types";
...
function resolveCommitteeConfig(formData: Partial<Agent>): CommitteeConfig {
  ...
  return {
    ...
    seats: candidate.seats?.length ? candidate.seats : structuredClone(DEFAULT_COMMITTEE_CONFIG.seats),
  };
}
...
export function CommitteeTab({ formData, updateField, availableModels }: CommitteeTabProps) {
  const [isRunning, setIsRunning] = useState(false);
  const [validationHeadline, setValidationHeadline] = useState<string | null>(null);
  ...
  const updateSeat = (seatKey: string, updates: Partial<CommitteeSeatConfig>) => {
    persistCommittee({
      ...committee,
  ...
  const updateOrchestrator = (field: keyof CommitteeConfig["orchestrator"], value: string) => {
    persistCommittee({
      ...committee,
      orchestrator: {
  ...
  const runValidation = async () => {
    ...
    try {
      ...
      setValidationHeadline(payload.committee_summary?.headline ?? "Validation completed.");
    } catch (error) {
    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-amber-500/20 bg-[radial-gradient(circle_at_top_left,_rgba(245,158,11,0.16),_transparent_32%),linear-gradient(180deg,rgba(17,24,39,0.96),rgba(2,6,23,0.96))] p-6 shadow-[0_0_30px_-16px_rgba(245,158,11,0.5)]">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <h3 className="mt-2 text-2xl font-semibold text-slate-100">Market Prediction Committee</h3>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
              Configure the roundtable roster that Portfolio AI uses for market forecasts, then validate the live seat mix against the current backend committee endpoint.
            </p>
          </div>
          <button
            type="button"
        ...
        {validationResult ? (
          <div className="mt-4 rounded-2xl border border-cyan-500/20 bg-cyan-500/10 p-4 text-sm text-cyan-50">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-3">
              <div>
                <p className="text-[11px] uppercase tracking-[0.24em] text-cyan-200/70">Committee bias</p>
                <p className="mt-1 text-sm font-medium text-cyan-50">
                  {validationResult.committee_summary?.headline ?? validationHeadline ?? "Validation completed."}
                </p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-[0.24em] text-cyan-200/70">Seat count</p>
                <p className="mt-1 text-sm font-medium text-cyan-50">
                  {validationResult.committee_summary?.seat_count ?? validationResult.votes?.length ?? 0}
                </p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-[0.24em] text-cyan-200/70">Disagreement</p>
                <p className="mt-1 text-sm font-medium text-cyan-50">
                  {validationResult.committee_summary?.disagreement_label ?? "n/a"}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          ...
          <label className="space-y-2 text-sm text-slate-300">
            ...
            <input
              ...
              onChange={(event) => updateOrchestrator("agent_slug", event.target.value)}
          ...
          <label className="space-y-2 text-sm text-slate-300">
            ...
            <select
              ...
              onChange={(event) => updateOrchestrator("model_id", event.target.value)}
              className="w-full rounded-2xl border border-slate-800 bg-slate-950 px-3 py-2 text-slate-100"
            >
          ...
          <label className="space-y-2 text-sm text-slate-300 xl:col-span-1">
            ...
            <textarea
              ...
              onChange={(event) => updateOrchestrator("instruction", event.target.value)}
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        {committee.seats.map((seat) => (
          <section key={seat.key} className="rounded-3xl border border-slate-800/80 bg-slate-950/60 p-5">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="section-kicker">Seat</p>
                <h4 className="mt-2 text-lg font-semibold text-slate-100">{seat.label}</h4>
              </div>
              <label className="inline-flex items-center gap-2 text-sm text-slate-300">
                <input
                  type="checkbox"
                  checked={seat.enabled}
                  onChange={(event) => updateSeat(seat.key, { enabled: event.target.checked })}
                />
                Enabled
              </label>
            </div>
            ...
            <div className="mt-4 space-y-4">
              <label className="space-y-2 text-sm text-slate-300">
                ...
                <input
                  ...
                  onChange={(event) => updateSeat(seat.key, { agent_slug: event.target.value })}
                  className="w-full rounded-2xl border border-slate-800 bg-slate-950 px-3 py-2 text-slate-100"
                />
              </label>
              ...
              <label className="space-y-2 text-sm text-slate-300">
                ...
                <select
                  ...
                  value={seat.model_id ?? ""}
                  onChange={(event) => updateSeat(seat.key, { model_id: event.target.value || null })}
                  className="w-full rounded-2xl border border-slate-800 bg-slate-950 px-3 py-2 text-slate-100"
                >
                  <option value="">Use agent default</option>
                  {availableModels.map((model) => (
              ...
              <label className="space-y-2 text-sm text-slate-300">
                ...
                <textarea
                  ...
                  onChange={(event) => updateSeat(seat.key, { instruction: event.target.value })}
                  className="min-h-[112px] w-full rounded-2xl border border-slate-800 bg-slate-950 px-3 py-2 text-slate-100"
                />
              </label>
            </div>
