...
import { formatPercent, formatRelativeTime, summarizeArenaIssue } from "@/app/arena/utils";
import {
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <dl className="mt-3 space-y-3">
            <div>
              <dt className="text-[11px] uppercase tracking-wide text-slate-500">{labLabel}</dt>
              <dd className="mt-1 text-xl font-semibold text-slate-100">{labValue}</dd>
            </div>
            <div>
              <dt className="text-[11px] uppercase tracking-wide text-slate-500">{fieldLabel}</dt>
              <dd className="mt-1 text-xl font-semibold text-slate-100">{fieldValue}</dd>
            </div>
          </dl>
...
function AttemptRow({ attempt }: { attempt: AgentBenchmarkAttemptDetail }) {
  return (
    ...
    >
      ...
      <span className="shrink-0 text-xs text-slate-400">{attempt.model_id}</span>
            ...
            return (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs">
                    ...
                    <span className="rounded-full bg-amber-950/30 px-2 py-1 text-amber-300 ring-1 ring-amber-900">
                      {formatMetricTokens(run.tokens_per_passed_attempt)} tok / pass
                    </span>
                ...
                <dl className="mt-4 grid grid-cols-2 gap-3 text-sm xl:grid-cols-4">
                  <div>
                    <dt className="text-xs uppercase tracking-wide text-slate-400">Cases</dt>
                    <dd className="mt-1 font-medium text-slate-100">{run.case_ids.length}</dd>
                  </div>
                  <div>
                    <dt className="text-xs uppercase tracking-wide text-slate-400">Passed</dt>
                  ...
                  <div>
                    <dt className="text-xs uppercase tracking-wide text-slate-400">Failures</dt>
                    <dd className="mt-1 font-medium text-slate-100">{run.failure_count ?? 0}</dd>
                  </div>
                </dl>
      ...
      {!hasScheduleRisks && !hasFieldRisks && !hasLabRisks ? (
        ...
        <div className="space-y-3">
          ...
          {dashboard.field_risks.map((item) => (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-semibold text-slate-100">Real heartbeat</p>
                <span
                  className={`text-xs font-medium ${
                    item.critical ? "text-amber-300" : "text-slate-400"
                  }`}
              ...
              <p className="mt-2 text-sm text-slate-300">{item.issue_summary}</p>
              <p className="mt-2 text-xs text-slate-500">
                {item.summary_oneliner ? summarizeArenaIssue(item.summary_oneliner, 120) : "No summary"}
              </p>
              <p className="mt-2 text-xs text-slate-500">
          ...
          {dashboard.open_regressions.map((item) => (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-semibold text-slate-100">{item.case_id}</p>
...
export function PersonaImprovementDashboard() {
  ...
  const cadenceLabel = useMemo(() => {
    const option = CADENCE_OPTIONS.find((candidate) => candidate.value === cadenceMinutes);
    if (option) {
      return option.label;
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">
                Focused on protected lab wins and recent real-heartbeat field performance.
              </p>
            </div>
          </div>
            ...
            <div className="space-y-4">
              ...
              <dl className="grid grid-cols-2 gap-3 text-sm">
                <div className="rounded-xl bg-slate-950/70 px-3 py-3">
                  <dt className="text-xs uppercase tracking-wide text-slate-400">Next run</dt>
                  <dd className="mt-1 font-medium text-slate-100">
                ...
                <div className="rounded-xl bg-slate-950/70 px-3 py-3">
                  ...
                  <dd className="mt-1 font-medium text-slate-100">{cadenceLabel}</dd>
                </div>
              </dl>

              <p className="text-xs text-slate-500">
                Last run {formatRelativeTime(dashboard.schedule.last_run_at)} · total runs{" "}
                {dashboard.schedule.run_count} · overlap skips instead of stacking
              </p>
              ...
              <button
                type="button"
                onClick={() => scheduleMutation.mutate({ enabled, cadence_minutes: cadenceMinutes })}
