...
function buildChartData(
  ...
  const getBucket = (key: string, completedAt: string | null, fallback: string) => {
    const existing = buckets.get(key);
    if (existing) {
      return existing;
    }
    const sortKey = completedAt ? new Date(completedAt).getTime() : Number.MAX_SAFE_INTEGER;
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                <XAxis dataKey="label" tick={{ fontSize: 11 }} stroke="#64748b" minTickGap={42} />
                <YAxis
                  domain={yDomain}
                  tick={{ fontSize: 11 }}
                  stroke="#64748b"
