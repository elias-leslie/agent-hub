...
function extractToolResult(toolOutput: unknown): string {
  if (typeof toolOutput === "string") return prettifyDisplayText(toolOutput) || "";
  if (!isRecord(toolOutput)) return "";
  const keys = ["content", "summary", "stdout", "message", "error_message", "stderr"];
  for (const key of keys) {
    const val = toolOutput[key];
    if (typeof val === "string" && val.trim()) return prettifyDisplayText(val) || "";
  }
  for (const val of Object.values(toolOutput)) {
    if (typeof val === "string" && val.trim() && val.length < 300) return prettifyDisplayText(val) || "";
  }
  return "";
}
...
function isNoiseEvent(event: TimelineEvent): boolean {
  ...
  if (event.event_type === "system_message" || event.event_type === "user_message") return true;
  if (event.event_type === "thinking") {
    const text = (event.content || "").toLowerCase();
    return !text.includes("error") && !text.includes("blocked") && !text.includes("failed");
  }
  if (event.event_type === "assistant_message") {
    if (isPromptLikeText(event.content) || isGenericStatusText(event.content)) return true;
...
function buildTranscriptItems(events: TimelineEvent[], narrationTags?: NarrationTag[]): TranscriptItem[] {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    if (event.event_type === "error") {
      processedIds.add(event.id);
      items.push({ id: event.id, timestamp: event.created_at, kind: "error",
        content: prettifyDisplayText(event.content || "Unknown error") || "Unknown error" });
      continue;
    }

    if ((event.event_type === "assistant_message" || event.event_type === "thinking") && event.content) {
      processedIds.add(event.id);
      items.push({ id: event.id, timestamp: event.created_at, kind: "message",
  ...
  if (narrationTags && narrationTags.length > 0) {
    for (const tag of narrationTags) {
      items.push({ id: `narration-${tag.id}`, timestamp: tag.created_at, kind: "narration",
        tagType: tag.tag_type, tagContent: tag.content });
    }
  }
...
const NARRATION_ICONS: Record<string, string> = {
  started: "▸", found: "🔍", modified: "✏️", tested: "✓",
  confidence: "📊", blocked: "⚠", decision: "⚖️",
};

// ─── SessionTranscript: chronological chat-like event stream ───

export function SessionTranscript({
  events, narrationTags, narrationLoading, issueMarkers,
  personaName, summaryText, hideRedundantSummaryMessage = false,
}: {
    ...
    <div className="mt-3 border-t border-slate-800/40 pt-3 space-y-2">
      {items.map((item) => {
              ...
              {item.toolResult && (
                <div className={cn("ml-4 mt-1 border-l border-slate-800/50 pl-2.5", item.isError ? "text-rose-400/80" : "text-slate-500")}>
                  {item.toolResultFull && !isExpanded ? (
                    <>{item.toolResult}<button onClick={(e) => { e.stopPropagation(); toggleExpand(item.id); }} className="ml-1.5 text-slate-600 hover:text-slate-400 font-sans text-[10px]">more</button></>
                  ) : item.toolResultFull && isExpanded ? (
        ...
        if (item.kind === "narration") {
                ...
                {pct !== null && (
                  <>
                    <div className="h-1.5 w-14 rounded-full bg-slate-800/60 overflow-hidden">
                      <div className={cn("h-full rounded-full transition-all", pct >= 70 ? "bg-emerald-500/70" : pct >= 40 ? "bg-amber-500/70" : "bg-rose-500/70")} style={{ width: `${Math.min(pct, 100)}%` }} />
                    </div>
          ...
          return (
            <div key={item.id} className={cn("text-xs py-0.5", item.tagType === "blocked" ? "text-rose-400/80" : "text-slate-400/80")}>
        ...
        <div className="mt-3 space-y-1.5 border-t border-slate-800/30 pt-2.5">
          {issueMarkers.map((marker) => (
            <div key={`issue-${marker.event_id}-${marker.primary_tag}`} className="flex items-start gap-2 text-xs text-amber-400/80">
              ...
              <div><span className="font-medium">{pulseTagLabel(marker.primary_tag)}:</span>{" "}{marker.title}</div>
            </div>
          ))}
        </div>
      )}
