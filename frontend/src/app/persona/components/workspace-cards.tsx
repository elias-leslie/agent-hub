...
import { Bot, ChevronDown, Clock3, HeartPulse, Loader2, Wrench } from "lucide-react";

import type { PersonaIssueMarker, PersonaStreamEntry } from "@/lib/api/persona-stream";
import { cn } from "@/lib/utils";
import { pulseTagClasses, pulseTagLabel, visibleIssueMarkers } from "./pulse-helpers";
import type {
  FeedChildRun,
    ...
    <div className="rounded-xl border border-slate-800/30 bg-slate-900/15 px-3.5 py-2.5">
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-xs font-medium text-slate-500">{block.items.length} routine heartbeats</span>
...
export function ChildRunCard({ entry, activeIssueTag, selected, expanded, onToggle, details, narrationTags, narrationLoading, personaName }: CardProps) {
  ...
  return (
    ...
    >
      ...
      <p className={cn("mt-1.5 text-xs text-slate-400/90 leading-relaxed", expanded ? "whitespace-pre-wrap" : "line-clamp-2")}>{summaryText}</p>
      {issueMarkers[0] && !expanded && <EntryIssueSummary issueMarkers={issueMarkers} />}
      {expanded && details && !details.loading && !details.error && (
        <SessionTranscript events={details.events} narrationTags={narrationTags} narrationLoading={narrationLoading}
          issueMarkers={issueMarkers} personaName={entry.agent_slug || personaName} summaryText={summaryText} hideRedundantSummaryMessage />
      )}
      {expanded && details?.loading && <TranscriptLoading />}
      {expanded && details?.error && <TranscriptError message={details.error} />}
    </div>
  );
}
...
export function HeartbeatCard({ entry, activeIssueTag, selected, expanded, onToggle, details, narrationTags, narrationLoading, personaName }: CardProps) {
  ...
  return (
    ...
    >
      ...
      {expanded && details && !details.loading && !details.error && (
        <SessionTranscript events={details.events} narrationTags={narrationTags} narrationLoading={narrationLoading}
          issueMarkers={issueMarkers} personaName={personaName} summaryText={summaryText} hideRedundantSummaryMessage />
      )}
      {expanded && details?.loading && <TranscriptLoading />}
      {expanded && details?.error && <TranscriptError message={details.error} />}
    </div>
