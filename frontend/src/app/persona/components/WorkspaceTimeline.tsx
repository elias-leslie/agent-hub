...
import type { PersonaPulseSummary, PersonaPulseMetric } from "@/lib/api/persona-stream";
import type { SessionEventDetailsState, TimelineRow, FeedAnchor, FeedChildRun } from "./workspace-types";
...
type NarrationCache = Record<string, { tags: NarrationTag[]; loading: boolean; error: string | null }>;
...
interface WorkspaceTimelineProps {
  ...
  fetchNarrationTags: (externalId: string) => Promise<void>;
  ...
  onToggleEntry: (entryId: string, sessionId?: string, externalId?: string) => void;
              ...
              {renderedRows.map(({ key, row, start, measure }) => {
                ...
                return (
                  <TimelineRowRenderer
                    ...
                    narrationCache={narrationCache}
                    fetchNarrationTags={fetchNarrationTags}
...
function TimelineRowRenderer({
  ...
  const wrapperStyle = renderVirtualRows ? { transform: `translateY(${start}px)` } : undefined;
      ...
      <div ref={measureRef} className={wrapperClass} style={wrapperStyle}>
        <RoutineHeartbeatGroup
          ...
          onToggleEntry={(entryId, sessionId) => {
            ...
            onToggleEntry(entryId, sessionId, entry?.anchorItem.entry.external_id ?? undefined);
          }}
          sessionEventDetails={sessionEventDetails}
          narrationCache={narrationCache}
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <ChildRunCard
              ...
              onToggle={() => onToggleEntry(item.id, item.sessionId, item.entry.external_id ?? undefined)}
            ...
            {row.childRuns.length > 0 && (
              <ChildRunStack
                ...
                onToggle={(entryId, sessionId) => {
                  ...
                  onToggleEntry(entryId, sessionId, cr?.entry.external_id ?? undefined);
                }}
                matchedIds={matchedIds}
                activeMatchId={activeMatchId}
                sessionEventDetails={sessionEventDetails}
