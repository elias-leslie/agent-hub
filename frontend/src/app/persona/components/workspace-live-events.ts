import type { PersonaStreamEntry, PersonaStreamEventPreview } from "@/lib/api/persona-stream";
...
import { shortenText, heartbeatIsImportant } from "./workspace-utils";
...
function safeObjectField(data: unknown, field: string): Record<string, unknown> | null {
  if (typeof data !== "object" || data === null || !(field in data)) return null;
  const val = (data as Record<string, unknown>)[field];
...
export function buildLivePreview(event: LiveSessionEvent): PersonaStreamEventPreview | null {
  const previewId = `live:${event.session_id}:${event.timestamp}:${event.event_type}`;

  if (event.event_type === "message") {
    return {
      id: previewId, event_type: safeStringField(event.data, "role") ? `${safeStringField(event.data, "role")}_message` : "assistant_message",
      created_at: event.timestamp, role: safeStringField(event.data, "role"), tool_name: null,
      content_preview: safeStringField(event.data, "content"), tool_input_preview: null,
      tool_output_preview: null, duration_ms: null, model_used: null,
    };
  }
  if (event.event_type === "tool_use") {
    return {
      id: previewId, event_type: "tool_use", created_at: event.timestamp, role: null,
      tool_name: safeStringField(event.data, "tool_name"), content_preview: null,
      tool_input_preview: safeObjectField(event.data, "tool_input") ? JSON.stringify(safeObjectField(event.data, "tool_input")) : null,
      tool_output_preview: safeObjectField(event.data, "tool_output") ? JSON.stringify(safeObjectField(event.data, "tool_output")) : null,
      duration_ms: null, model_used: null,
    };
  }
  if (event.event_type === "tool_result") {
    const toolOutput = safeObjectField(event.data, "tool_output");
    return {
      id: previewId, event_type: "tool_result", created_at: event.timestamp, role: null,
      tool_name: safeStringField(event.data, "tool_name"),
      content_preview: toolOutput ? safeStringField(toolOutput, "content") : null,
      tool_input_preview: null,
      tool_output_preview: toolOutput ? JSON.stringify(toolOutput) : null,
      duration_ms: safeNumberField(event.data, "duration_ms"), model_used: null,
    };
  }
  if (event.event_type === "error") {
    return {
      id: previewId, event_type: "error", created_at: event.timestamp, role: null, tool_name: null,
      content_preview: safeStringField(event.data, "error_message") || "Session error",
      tool_input_preview: null, tool_output_preview: null, duration_ms: null, model_used: null,
    };
  }
  if (event.event_type === "complete") {
    return {
      id: previewId, event_type: "tool_result", created_at: event.timestamp, role: null, tool_name: null,
      content_preview: "Execution completed", tool_input_preview: null, tool_output_preview: null,
      duration_ms: null, model_used: null,
    };
  }
  return null;
...
export function liveSummaryFromEvent(event: LiveSessionEvent): string | null {
  ...
  if (event.event_type === "tool_result") {
    ...
    const isError = safeBoolField(event.data, "is_error") ?? (toolOutput ? safeBoolField(toolOutput, "is_error") : false) ?? false;
  ...
  if (event.event_type === "error") {
    const errorMessage = safeStringField(event.data, "error_message") || "Session error";
...
export function isRoutineHeartbeatBlock(
  ...
  if (entry.status !== "completed" || entry.live_status === "active") return false;
