...
import { type LiveSessionPatch, EMPTY_PULSE, PAGE_SIZE } from "./workspace-types";
...
function mergeEntries(prev: PersonaStreamEntry[], next: PersonaStreamEntry[]): PersonaStreamEntry[] {
...
function applyLivePatch(entry: PersonaStreamEntry, patch: LiveSessionPatch): PersonaStreamEntry {
  return {
    ...entry,
    ...
    status:
      patch.liveStatus === "failed"
        ...
        : patch.liveStatus === "completed" && entry.status === "active"
          ? "completed"
          : entry.status,
    event_previews: mergeEventPreviewsInline(entry.event_previews, patch.eventPreviews),
  };
}
...
function mergeEventPreviewsInline(
  ...
  for (const update of updates) {
    const index = merged.findIndex((item) => item.tool_name === update.tool_name);
    if (index >= 0) {
      merged[index] = update;
    } else {
      merged.push(update);
...
function filterFeedItems(items: FeedItem[], filterMode: FilterMode): FeedItem[] {
  if (filterMode === "all") return items;
  if (filterMode === "messages") return items.filter((item) => item.kind === "message");
  if (filterMode === "heartbeats") return items.filter((item) => item.kind === "heartbeat");
  if (filterMode === "work") return items.filter((item) => item.kind === "child_run" || item.kind === "heartbeat");
...
export function usePersonaFeed({
  ...
  useEffect(() => {
    ...
    fetchPersonaStream({
      ...
      .then((data) => {
        if (cancelled) return;
        startTransition(() => {
          setEntries((prev) => (page === 1 ? data.entries : mergeEntries(prev, data.entries)));
          setTotal(data.total);
  ...
  const mergedItems = useMemo(() => {
    ...
    const allItems = [...remote, ...local].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
    return filterFeedItems(allItems, filterMode);
  }, [hydratedEntries, messages, currentSessionId, activeSessionId, filterMode, personaDisplayName]);
