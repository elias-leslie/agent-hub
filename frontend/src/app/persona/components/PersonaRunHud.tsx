...
import { shortenText, prettifyDisplayText } from "./workspace-format";
...
export function PersonaRunHud({
  ...
  const meta = [
    ...
    details?.context_usage ? `context ${Math.round(details.context_usage.percent_used)}%` : null,
    ...
    <div data-testid="persona-run-hud" className={shellClassName}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          {summary ? <div className="text-sm font-medium text-slate-100">{summary}</div> : null}
          {meta.length > 0 ? <div className="mt-2 text-xs text-slate-500">{meta.join(" · ")}</div> : null}
        </div>
