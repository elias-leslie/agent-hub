...
import { isChildRunItem, canAnchorChildRuns } from "./workspace-feed";
  ...
  for (const group of groupedItems) {
    ...
    for (const block of group.blocks) {
      if (block.kind === "routine_group") {
        ...
        if (containsUnread) rows.push({ kind: "unread", id: `unread:${block.id}` });
...
function getStreamItemIds(row: TimelineRow): string[] {
  ...
  if (row.kind === "routine_group") {
    return row.block.items.flatMap(({ anchorItem, childRuns }) => [anchorItem.id, ...childRuns.map((cr) => cr.id)]);
  }
  return [];
}
