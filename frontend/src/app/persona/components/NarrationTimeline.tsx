...
const TAG_CONFIG: Record<string, { icon: typeof Play; label: string; color: string }> = {
  ...
  confidence: { icon: BarChart3, label: "Confidence", color: "text-violet-400" },
            ...
            <div key={tag.id} className="flex items-start gap-2 relative">
              ...
              <div className="min-w-0 flex-1">
                {tag.tag_type === "confidence" ? (
                  ...
                  <p className="text-xs text-slate-300 leading-relaxed">{tag.content}</p>
                )}
              </div>
            </div>
          );
