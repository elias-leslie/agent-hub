...
export function describeValue(value: unknown): string | null {
  ...
  if (typeof value === "string") { const p = prettifyDisplayText(value); return p || null; }
  if (typeof value === "number" || typeof value === "boolean") return String(value);
  if (Array.isArray(value)) {
    const items = value.map((item) => describeValue(item)).filter((item): item is string => Boolean(item));
...
function fieldTone(label: string, value: string): DetailField["tone"] {
  ...
  if (normalizedLabel.includes("status")) {
    if (["ok", "success", "succeeded", "completed"].includes(normalizedValue)) return "success";
    if (["failed", "error", "blocked"].includes(normalizedValue)) return "danger";
  }
  if (normalizedLabel.includes("exit code")) return normalizedValue === "0" ? "success" : "danger";
  if (normalizedValue === "true" && normalizedLabel.includes("error")) return "danger";
  if (normalizedLabel.includes("file") || normalizedLabel.includes("path")) return "info";
  if (normalizedLabel.includes("task")) return "warning";
  return "neutral";
}

function flattenReadableFields(value: unknown, parentKey = "", depth = 0): DetailField[] {
  ...
  const preferredOrder = [
    "command", "action", "description", "path", "file_path", "target_file",
    "task_id", "project", "project_id", "status", "exit_code",
    "current_branch", "branch", "files_touched", "changed_files",
  ];
...
export function buildEventDetails(event: TimelineEvent): {
  ...
  const addField = (label: string, value: string | null | undefined, tone?: DetailField["tone"]) => {
    ...
    fields.push({ label, value: trimmed, tone: tone ?? fieldTone(label, trimmed) });
  };

  const recordTextFields = (value: Record<string, unknown> | null, keys: string[]) => {
    if (!value) return;
    for (const key of keys) addUniqueText(textBlocks, textSeen, describeValue(value[key]));
  };

  const filterFields = (source: Record<string, unknown> | null, excludedKeys: string[]) => {
    if (!source) return;
    ...
    for (const field of flattenReadableFields(source)) {
      ...
      const value = field.label.toLowerCase().includes("file") || field.label.toLowerCase().includes("path")
        ? compactPath(field.value) : field.value;
  ...
  if (event.duration_ms != null) addField("Duration", formatDurationLabel(event.duration_ms));
...
export function eventImportanceScore(event: TimelineEvent, details: ReturnType<typeof buildEventDetails>): number {
  ...
  if (event.event_type === "memory_inject" || event.event_type === "memory_cite") return 0;
  if ((event.event_type === "system_message" || event.role === "system") && isPromptLikeText(event.content)) return 0;
  if (event.event_type === "thinking") return combinedText.includes("error") || combinedText.includes("blocked") ? 40 : 5;
  if (event.event_type === "tool_result") {
    const hasFailureSignal = combinedText.includes("error") || combinedText.includes("failed")
      || fieldText.includes("exit code 1") || fieldText.includes("status failed") || fieldText.includes("error true");
    if (hasFailureSignal) return 95;
    const hasStrongOutcome = combinedText.includes("publish") || combinedText.includes("commit")
      || combinedText.includes("verified") || combinedText.includes("merged") || combinedText.includes("completed")
      || combinedText.includes("passed") || fieldText.includes("task") || fieldText.includes("files touched");
    if (hasStrongOutcome) return 82;
    return isGenericStatusText(details.lead) ? 20 : 55;
  }
  if (event.event_type === "tool_use") {
    const hasActionSignal = combinedText.includes("publish") || combinedText.includes("commit")
      || combinedText.includes("verify") || combinedText.includes("dispatch")
      || fieldText.includes("task") || fieldText.includes("file");
...
export function buildSessionDetailBlocks(events: TimelineEvent[]): SessionDetailBlock[] {
  ...
  for (let index = 0; index < events.length; index += 1) {
    ...
    if (
      ...
      const mergedText = dedupeTextBlocks(
        [resultDetails.lead, useDetails.lead, ...resultDetails.textBlocks, ...useDetails.textBlocks]
...
export function blockMatchesIssueMarker(block: SessionDetailBlock, marker: PersonaIssueMarker): boolean {
  if (block.id === marker.event_id || block.id.split(":").includes(marker.event_id)) return true;
  const haystack = `${block.title ?? ""} ${block.lead ?? ""} ${block.textBlocks.join(" ")}`.toLowerCase();
  if (marker.tool_name && haystack.includes(marker.tool_name.toLowerCase())) return true;
  const markerWords = `${marker.title} ${marker.summary} ${marker.detail ?? ""}`
    .toLowerCase().split(/\s+/).filter((token) => token.length > 4);
