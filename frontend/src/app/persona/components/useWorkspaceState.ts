...
import {
  buildLivePreview,
  liveStatusFromEvent,
  liveSummaryFromEvent,
  mergeEventPreviews,
} from './workspace-live-events'
    ...
    new Set(
      mergedItems
        .filter(
          (
            item,
          ): item is Extract<FeedItem, { kind: 'heartbeat' | 'child_run' }> =>
            item.kind !== 'message',
        )
        .filter(
          (item) =>
            expandedEntryIds[item.id] &&
            activeStreamSessionIds.includes(item.sessionId),
        )
...
export function useWorkspaceState({
  ...
  >({})
  const [expandedRoutineGroupIds, setExpandedRoutineGroupIds] = useState<
    Record<string, boolean>
  >({})
  const [liveRefreshTick, setLiveRefreshTick] = useState(0)
  const [liveSessionPatches, setLiveSessionPatches] = useState<
    Record<string, LiveSessionPatch>
  >({})
  ...
  const { sessionEventDetails, loadSessionEventDetails } =
    useSessionEventDetails()
      ...
      setLiveSessionPatches((current) => {
        ...
        return {
          ...
          [event.session_id]: {
            ...
            eventPreviews: preview
              ? mergeEventPreviews(existing.eventPreviews, [preview])
              : existing.eventPreviews,
          },
        }
      })
  ...
  useEffect(() => {
    ...
    const ids = getExpandedActiveSessionIds(
      mergedItems,
      expandedEntryIds,
      activeStreamSessionIds,
    )
  ...
  useEffect(() => {
    ...
    if (
      hydratedEntries.some(
        (entry) => entry.id === activeMatchId && entry.entry_type !== 'message',
      )
    ) {
      setExpandedEntryIds((current) =>
        current[activeMatchId]
          ? current
          : { ...current, [activeMatchId]: true },
      )
    }
    ...
    const timeout = window.setTimeout(() => {
      document
        .querySelector<HTMLElement>(`[data-stream-item-id="${activeMatchId}"]`)
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      const nodes = document.querySelectorAll<HTMLElement>(
        `[data-session-anchor="${focusSessionId}"]`,
      )
  ...
  const newActivityCount = useMemo(() => {
    ...
    const unreadIndex = mergedItems.findIndex(
      (item) => item.id === firstUnreadItemId,
    )
  ...
  const jumpToSearchMatch = (direction: 1 | -1) => {
    ...
    const currentIndex = searchMatches.findIndex(
      (item) => item.entry_id === activeMatchId,
    )
    const resolvedIndex = currentIndex >= 0 ? currentIndex : 0
    const nextIndex = resolvedIndex + direction
    const wrappedEntry =
      nextIndex < 0
        ? searchMatches.at(-1)?.entry_id
        : nextIndex >= searchMatches.length
          ? searchMatches[0]?.entry_id
  ...
  return {
    ...
    activeIssueTag:
      filterMode === 'friction'
        ? ('friction' as const)
        : filterModeToPulseTag(filterMode),
    handleSessionJump,
    toggleExpanded,
    toggleRoutineGroup,
    applyPulseFilter,
