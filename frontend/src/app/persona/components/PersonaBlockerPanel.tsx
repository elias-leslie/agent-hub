...
export function PersonaBlockerPanel({
  ...
  const permissionBlocker =
    executionPermission && !executionPermission.allowed
      ? executionPermission.reason
      : null
  const hardBlocker =
    liveActivity?.stall_reason || runtime.error || permissionBlocker || null
  const advisory =
    topIssue?.summary ||
    ...
    },
    {
      ...
      value: executionPermission
        ...
        : previewLoading
          ? 'Loading…'
          : 'Unavailable',
      tone: executionPermission?.allowed
        ? 'default'
        : executionPermission
          ? 'danger'
          : 'default',
  ...
  return (
    <EvidencePanel data-testid="persona-blocker-panel" className="p-4">
      <div className="flex items-center justify-between gap-3">
        <SectionEyebrow
          label="Blockers"
          source={hardBlocker ? 'runtime' : advisory ? 'advisory' : 'session'}
        />
        <button
          type="button"
          onClick={onRefresh}
        ...
        >
          <RefreshCw
            className={previewLoading ? 'h-4 w-4 animate-spin' : 'h-4 w-4'}
          />
        </button>
      </div>
      ...
      <div className="mt-3 space-y-2">
        {rows.map((row) => (
          <div
            key={row.label}
            className={`border-l-2 pl-3 ${lineTone(row.tone)}`}
          >
